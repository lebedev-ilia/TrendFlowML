# Audit v4.2 — engineering log: `cosine_metrics_extractor`

**Дата:** 2026-04-14  
**Компонент:** `cosine_metrics_extractor` (TextProcessor; табличный срез `tp_cos_*` в `text_processor/text_features.npz`)

## Цель

Подготовить контур Audit v4 **L2** (A+B) для `tp_cos_*`: сводка по табличному слою и качеству `result_store`.

## Инструментарий

- Скрипт: `DataProcessor/TextProcessor/src/extractors/cosine_metrics_extractor/scripts/audit_v4_npz_stats.py`
- Выход: `storage/audit_v4/cosine_metrics_extractor_l2/cosine_metrics_extractor_audit_v4_stats.json`
...
- **2/5** файлов `text_features.npz` имеют `meta.status=ok` и содержат полный срез `tp_cos_*` (**39** ключей)
