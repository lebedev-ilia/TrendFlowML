...
- **`core_object_detections`** остаётся **proposal generator** (геометрия + базовые классы таксономии v1). Baseline Audit v3: tracking удалён.
- “Богатая семантика” делается отдельными **semantic heads** поверх proposals (и surrogate track-агрегаций при необходимости):
...
- **Tracking (baseline)**: в Audit v3 tracking удалён из `core_object_detections`; head’ы используют surrogate `track_ids` per-detection (или frame-level агрегации).
...
**Назначение**: предложения объектов + базовая семантика + стабильная геометрия (baseline: без tracking).

**Выход** (`rs_path/core_object_detections/detections.npz`, schema v2, упрощённо):
- `frame_indices (N,)`
- `times_s (N,)`
- `boxes (N, MAX, 4)` (xyxy)
- `boxes_norm (N, MAX, 4)`
...
- `meta` + `meta_json` с models_used и параметрами
...
- B2. Зависимости и конфиги:
  ...
  - фиксировать что baseline без tracking; future: добавить трекинг отдельно (без ломания NPZ контракта)
