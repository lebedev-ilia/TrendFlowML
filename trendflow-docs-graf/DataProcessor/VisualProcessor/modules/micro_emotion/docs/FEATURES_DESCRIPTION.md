...
## Audit v3 — model-facing tabular format

В `micro_emotion_npz_v3` video-level скаляры хранятся **таблично**:

- `feature_names[V]`: имена скалярных признаков (фиксированный набор)
- `feature_values[V]`: значения (float32), `NaN` если недоступно

Ключ `features` (dict) больше **не используется** как model-facing контракт.
