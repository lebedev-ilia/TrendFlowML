...
Канон для NPZ-структуры, wide CSV и валидатора: **`docs/FEATURE_DESCRIPTION.md`**.

Модуль извлекает фичи текста по результатам OCR и анализирует их взаимодействие с видео.

## Baseline (Audit v3)

- **Schema**: `text_scoring_npz_v2`
...
- Если OCR-артефакт не найден — модуль **не падает**, а возвращает валидный empty результат:
  - `text_present=false`
  - `meta.status="empty"`, `meta.empty_reason="dependency_missing"`
...
- **OCR**: внешний компонент/сервис (модуль не выполняет OCR).
- **numpy/scipy**: численные вычисления и signal processing.
