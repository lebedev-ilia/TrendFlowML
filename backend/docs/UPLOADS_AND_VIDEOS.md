...
## 4) Идемпотентность `POST .../upload/complete` (контракт)

HTTP-эндпоинты upload в составе **текущего** FastAPI-приложения могут отсутствовать (см. `GAPS_AND_ALIGNMENT.md` §3); ниже — зафиксированное поведение для реализации и ревью / портфолио.

- Клиент передаёт заголовок **`Idempotency-Key`** (как для `POST /api/runs`).
- Ключ интерпретируется в разрезе **аутентифицированного пользователя** (и при необходимости workspace/channel): пара `(user_id, Idempotency-Key)` однозначно сопоставляется с **одним** успешным завершением upload-сессии.
- Повторный **`POST .../upload/complete`** с тем же ключом при уже завершённом upload возвращает **тот же ответ** (и тот же `video_id` / идентификаторы сущностей), **без** повторного переноса файла и **без** дублирования записей, влияющих на биллинг/квоты.
- При параллельных запросах с одним ключом сервер обеспечивает **at-most-one** успешное завершение (например, уникальный индекс или транзакционная «запись результата по ключу»).

## 5) Видео‑идентификаторы
