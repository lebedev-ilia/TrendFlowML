1. Читаешь код компонента, его зависимости, связи с сдругими компонентами и тд. Критерии проверки:
    ...
    7. Контракт артефакта и совместимость (обязательно, Audit v3):
        - Точный путь/имя артефакта (npz/json/папка) и стабильность имени (важно для downstream).
        - **Схема обязана быть чёткой и включённой в общую систему схем**:
          - **Human schema**: `SCHEMA.md` рядом с компонентом (описание ключей, dtype/shape, tiers, required/optional).
          - **Machine schema**:
            - для VisualProcessor: `DataProcessor/VisualProcessor/schemas/<schema_version>.json` (реестр по `meta.schema_version`),
            - для остальных процессоров — аналогичный реестр добавляется по мере rollout, но требование “чёткая схема + версия” остаётся обязательным.
          - Схема обязана фиксировать: ключи NPZ, dtype/shape, **tiers** (`model_facing|analytics|debug`), required/optional.
          - Для audited компонентов рекомендуется `allow_extra_keys=false`, чтобы контракт не менялся “тихо”.
    ...
    12. Тестируемость:
        ...
        - Валидация артефакта:
          - базовая meta-валидация (`artifact_validator.validate_npz`)
          - schema-валидация (keys/dtype/shape) через общую систему схем для audited компонентов (fail-fast).
