# Аудит соответствия car_semantics требованиям baseline

**Дата проверки**: 2026-01-XX  
**Компонент**: `car_semantics` (Semantic head, v1)  
**Расположение**: `VisualProcessor/core/model_process/core_identity/car_semantics/`  
**Статус аудита**: ✅ **CLOSED** (2026-01-XX)

## Резюме

`car_semantics` — semantic head компонент для распознавания автомобилей в видео. Компонент использует детекции из `core_object_detections`, извлекает crops с паддингом, использует Embedding Service для поиска похожих автомобилей и возвращает per-track и per-frame top‑K идентификаций с иерархической информацией (make, model, segment).
...
- Запись `car_semantics.npz` выполняется через `np.savez_compressed` (атомарное сохранение не требуется для NPZ)
...
- Используется фиксированное имя артефакта: `ARTIFACT_FILENAME = "car_semantics.npz"`
- Сохраняется в `result_store/<platform_id>/<video_id>/<run_id>/car_semantics/car_semantics.npz`
...
Артефакт: `car_semantics.npz` содержит:
...
- `track_topk_ids (T, K) int32` — Top‑K автомобилей на трек
- `track_topk_scores (T, K) float32` — Similarity scores для треков
- `track_topk_makes (T, K) str` — Make (производитель) для top‑K
- `track_topk_models (T, K) str` — Model (модель) для top‑K
- `track_topk_segments (T, K) str` — Segment (класс) для top‑K
- `frame_topk_ids (N, K) int32` — Top‑K автомобилей на кадр (дедуплицированные)
- `frame_topk_scores (N, K) float32` — Similarity scores для кадров
- `semantic_label_names` — массив строк "id:name" для маппинга label_id → car_name
...
- `core_object_detections` **обязателен** и должен быть выполнен перед `car_semantics` (no-fallback)
...
- Фильтрация по классу `car` / `vehicle` / `automobile` (если доступен в taxonomy)
...
- Human-friendly demo: `quality_report/demo_car_semantics_quality.py` (если создан)
...
4. **Иерархическая информация**: Компонент извлекает make/model/segment из метаданных Embedding Service, но если метаданные отсутствуют, поля будут пустыми

## Update: baseline contracts compliance (2026-01-XX)

Изменения:
- Добавлен `ARTIFACT_FILENAME = "car_semantics.npz"`
...
- **README компонента**: `VisualProcessor/core/model_process/core_identity/car_semantics/README.md`
