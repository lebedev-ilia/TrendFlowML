#!/bin/bash
# Скрипт для запуска тестов uniqueness компонента на всех видео (последовательно).
...
CONFIG_PROFILE="${CONFIG_PROFILE:-"${BASE_DIR}/DataProcessor/configs/audit_v3/visual/profile_uniqueness.yaml"}"
...
echo "=========================================="
echo "Запуск тестов uniqueness компонента"
...
echo ""
echo "[0] Тестирование: ${SHORTEST_VIDEO} (test_uniqueness_shortest)"
echo "----------------------------------------"
...
LOG_FILE="/tmp/uniqueness_test_test_uniqueness_shortest.log"
if "${PYTHON}" "${MAIN_SCRIPT}" \
    --video-path "${video_path}" \
    --global-config "${GLOBAL_CONFIG}" \
    --profile-path "${CONFIG_PROFILE}" \
    --platform-id youtube \
    --video-id test_uniqueness_shortest \
    --run-id test_uniqueness_shortest \
    --output-dir "${RESULTS_DIR}" \
    > "${LOG_FILE}" 2>&1; then
    echo "✅ Успешно: test_uniqueness_shortest"
else
    echo "❌ Ошибка (exit code: $?): test_uniqueness_shortest"
...
for i in "${!VIDEOS[@]}"; do
    ...
    LOG_FILE="/tmp/uniqueness_test_${video_id}.log"
    if "${PYTHON}" "${MAIN_SCRIPT}" \
        --video-path "${video_path}" \
        --global-config "${GLOBAL_CONFIG}" \
        --profile-path "${CONFIG_PROFILE}" \
