#!/bin/bash
# Скрипт для запуска тестов text_scoring компонента на всех видео (последовательно).
...
CONFIG_PROFILE="${CONFIG_PROFILE:-"${BASE_DIR}/DataProcessor/configs/audit_v3/visual/profile_text_scoring.yaml"}"
...
echo "=========================================="
echo "Запуск тестов text_scoring компонента"
...
echo ""
echo "[0] Тестирование: ${SHORTEST_VIDEO} (test_text_scoring_shortest)"
echo "----------------------------------------"
...
LOG_FILE="/tmp/text_scoring_test_test_text_scoring_shortest.log"
if "${PYTHON}" "${MAIN_SCRIPT}" \
    --video-path "${video_path}" \
    --global-config "${GLOBAL_CONFIG}" \
    --profile-path "${CONFIG_PROFILE}" \
    --platform-id youtube \
    --video-id test_text_scoring_shortest \
    --run-id test_text_scoring_shortest \
    --output-dir "${RESULTS_DIR}" \
    > "${LOG_FILE}" 2>&1; then
    echo "✅ Успешно: test_text_scoring_shortest"
else
    echo "❌ Ошибка (exit code: $?): test_text_scoring_shortest"
...
for i in "${!VIDEOS[@]}"; do
    video="${VIDEOS[$i]}"
    video_id="test_text_scoring_$((i+2))"
    ...
    LOG_FILE="/tmp/text_scoring_test_${video_id}.log"
    if "${PYTHON}" "${MAIN_SCRIPT}" \
        --video-path "${video_path}" \
        --global-config "${GLOBAL_CONFIG}" \
        --profile-path "${CONFIG_PROFILE}" \
