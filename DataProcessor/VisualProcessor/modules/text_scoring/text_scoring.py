...
VERSION = "2.0"
SCHEMA_VERSION = "text_scoring_npz_v1"
...
class TextScoringModule(BaseModule):
    ...
    def process(self, frame_manager: FrameManager, frame_indices: List[int], config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not ocr_npz:
            ...
            features = {"text_present": False, "ocr_npz": None}
            return {
                "frame_indices": fi,
                "times_s": times_s,
        ...
        if not ocr_data:
            ...
            features = {"text_present": False, "ocr_npz": str(ocr_npz)}
            return {
                "frame_indices": fi,
                "times_s": times_s,
                "text_present": np.asarray(False),
        ...
        if not ocr_filtered:
            meta_override = {"status": "empty", "empty_reason": "no_text_available"}
            features = {"text_present": False, "ocr_npz": str(ocr_npz), "notes": "ocr_outside_sampling"}
            return {
                "frame_indices": fi,
                "times_s": times_s,
                "text_present": np.asarray(False),
