...
# Producer version for NPZ meta (Audit v3 baseline)
VERSION = "2.0.1"
...
class StressAnalyzer:
    ...
    def __init__(self, window_size=30):
        ...
        # Reuse a single classifier instance (avoid per-frame allocations).
    ...
    def analyze_stress(self, face_landmarks, pose_landmarks, hand_landmarks_list, image_shape):
        ...
        if hand_landmarks_list:
            for hand_landmarks in hand_landmarks_list:
                gesture = self.gesture_classifier.classify_gesture_hard(hand_landmarks, pose_landmarks)
...
class BehaviorAnalyzer(BaseModule):
    ...
    def _pack_npz_results(
        ...
        *,
        ...
        for i, frame_idx in enumerate(frame_indices):
            ...
            if store_debug_objects:
                frame_results.append(self.make_serializable(res))
                hand_gestures.append(res.get("hand_gestures", []))
            else:
                # Keep required keys for schema validation, but reduce artifact size for production runs.
        ...
        # Ensure hand_gestures is always 1D array with correct length (fix for empty lists case)
        if len(hand_gestures) != n:
            # Pad or truncate to match n
            if len(hand_gestures) < n:
                hand_gestures.extend([[]] * (n - len(hand_gestures)))
            else:
                hand_gestures = hand_gestures[:n]
        # Create 1D object array explicitly to avoid 2D array when all lists are empty
        hand_gestures_arr = np.empty(n, dtype=object)
        for i in range(n):
            hand_gestures_arr[i] = hand_gestures[i] if i < len(hand_gestures) else []
        
        packed: Dict[str, Any] = {
            "frame_indices": np.asarray(frame_indices, dtype=np.int32),
            "times_s": np.asarray(times_s, dtype=np.float32),
            "landmarks_present": np.asarray(landmarks_present, dtype=bool),
            "hand_gestures": hand_gestures_arr,
    ...
    def _aggregate_results(self, results: Dict[int, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Скорость смены жестов: считаем по dominant gesture per-frame (по soft распределению),
        # а не по "плоскому" списку all_gestures (который смешивает руки/кадры).
        dominant_per_frame: List[Optional[str]] = []
        for s in seq_list:
            probs = s.get("gesture_probs", {}) if isinstance(s, dict) else {}
            if not isinstance(probs, dict) or not probs:
                dominant_per_frame.append(None)
                continue
            best_g = None
            best_p = -1e9
            for g, p in probs.items():
                try:
                    pv = float(p)
                    if np.isfinite(pv) and pv > best_p:
                        best_p = pv
                        best_g = str(g)
                except Exception:
                    continue
            dominant_per_frame.append(best_g)

        switches = 0
        valid_pairs = 0
        for i in range(1, len(dominant_per_frame)):
            a = dominant_per_frame[i - 1]
            b = dominant_per_frame[i]
            if a is None or b is None:
                continue
            valid_pairs += 1
            if a != b:
                switches += 1
        aggregated["gesture_switching_rate"] = float(switches / max(valid_pairs, 1))
        ...
        stress_contrast, early_stress, late_stress = temporal_contrast(stress_proxy)
        _speech_contrast, early_speech, late_speech = temporal_contrast(speech_proxy)
        ...
        aggregated['early_late_ratios'] = {
            ...
            'speech_activity': float((late_speech + 1e-6) / (early_speech + 1e-6)),
    ...
    def run(
        ...
        try:
            ...
            cfg = config or {}
            store_debug_objects = bool(cfg.get("store_debug_objects", True))
            packed = self._pack_npz_results(
                results=results,
                frame_indices=frame_indices,
                times_s=times_s,
                landmarks_present=landmarks_present,
                store_debug_objects=store_debug_objects,
            )
            ...
            # Audit v3: stage timings (human QA + reproducibility)
            save_metadata["stage_timings_ms"] = {
                "process": int(round((t_process_end - t_process_start) * 1000.0)),
                "pack": int(round((t_pack_end - t_pack_start) * 1000.0)),
                # Note: save() timing is not included because NPZ meta is written during save().
                "total_without_save": int(round((time.time() - t_run_start) * 1000.0)),
            }
            ...
            # Update total timing to include save() (store separately to avoid mutating already-saved meta)
            try:
                save_ms = int(round((time.time() - t_save_start) * 1000.0))
                total_ms = int(round((time.time() - t_run_start) * 1000.0))
                self.logger.info(
                    f"{self.module_name} | stage_timings_ms: "
                    f"process={save_metadata['stage_timings_ms'].get('process')}ms, "
                    f"pack={save_metadata['stage_timings_ms'].get('pack')}ms, "
                    f"save={save_ms}ms, "
                    f"total={total_ms}ms"
                )
            except Exception:
