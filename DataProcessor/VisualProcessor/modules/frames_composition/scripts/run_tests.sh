#!/bin/bash
# Скрипт для запуска тестов frames_composition компонента на всех видео
...
CONFIG_PROFILE="${CONFIG_PROFILE:-"${BASE_DIR}/DataProcessor/configs/audit_v3/visual/profile_frames_composition.yaml"}"
...
echo "=========================================="
echo "Запуск тестов frames_composition компонента"
echo "Всего видео: $(( ${#VIDEOS[@]} + 1 ))"
echo "=========================================="

# Тест на самом коротком видео
echo ""
echo "[0] Тестирование: ${SHORTEST_VIDEO} (test_frames_composition_shortest)"
echo "----------------------------------------"
...
LOG_FILE="/tmp/frames_composition_test_test_frames_composition_shortest.log"
if "${PYTHON}" "${MAIN_SCRIPT}" \
    --video-path "${video_path}" \
    --global-config "${GLOBAL_CONFIG}" \
    --profile-path "${CONFIG_PROFILE}" \
    --platform-id youtube \
    --video-id test_frames_composition_shortest \
    --run-id test_frames_composition_shortest \
    --output-dir "${RESULTS_DIR}" \
    > "${LOG_FILE}" 2>&1; then
    echo "✅ Успешно: test_frames_composition_shortest"
else
    echo "❌ Ошибка (exit code: $?): test_frames_composition_shortest"
...
for i in "${!VIDEOS[@]}"; do
    video="${VIDEOS[$i]}"
    video_id="test_frames_composition_$((i+2))"
    ...
    LOG_FILE="/tmp/frames_composition_test_${video_id}.log"
    if "${PYTHON}" "${MAIN_SCRIPT}" \
        --video-path "${video_path}" \
        --global-config "${GLOBAL_CONFIG}" \
        --profile-path "${CONFIG_PROFILE}" \
