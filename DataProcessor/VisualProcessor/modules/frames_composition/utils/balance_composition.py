...
def _load_core_faces(d: Dict[str, Any], want_fi: np.ndarray) -> CoreFaces:
    ...
    # Some core_face_landmarks versions produce shape (N, 1, 468, 3) instead of (N, 468, 3).
    # Normalize to the baseline contract: (N, 468, 3).
    if face.ndim == 4 and face.shape[1] == 1 and face.shape[2] == 468 and face.shape[3] == 3:
        face = face[:, 0, :, :]
...
def _load_core_depth(d: Dict[str, Any], want_fi: np.ndarray) -> CoreDepth:
    meta = _require_npz_meta(d, "core_depth_midas")
    status = str(meta.get("status") or "error")
    if status != "ok":
        raise RuntimeError(f"{MODULE_NAME} | core_depth_midas must be status=ok (no-fallback). Got: {status}")
    fi = np.asarray(d.get("frame_indices"), dtype=np.int32).reshape(-1)
...
def _crop_letterbox_rgb(frame_rgb: np.ndarray, *, black_thr: float = 0.03, min_crop_frac: float = 0.02) -> np.ndarray:
    ...
    This improves stability of edge/symmetry/saliency metrics for many videos.
    """
    try:
...
class FramesCompositionModule(BaseModule):
    ...
    def get_models_used(self, config: Dict[str, Any], metadata: Dict[str, Any]) -> List[Dict[str, Any]]:
        # No ML models are loaded directly in this module (pure CV + consumes core providers).
        return []

    def process(self, frame_manager: FrameManager, frame_indices: List[int], config: Dict[str, Any]) -> Dict[str, Any]:
        t_start = time.time()
        # Saved into NPZ meta by save_results(); best-effort only.
        try:
            self._resource_profile_before = _resource_profile_snapshot()
        except Exception:
            self._resource_profile_before = {}
        stage_timings_ms: Dict[str, float] = {}

        t_stage = time.time()
        cfg = config or {}
        stage_timings_ms["init"] = (time.time() - t_stage) * 1000.0
        ...
        stage_timings_ms["axis"] = (time.time() - t_stage) * 1000.0
        ...
        if not has_any_face:
            ...
            frame_feature_present_ratio = np.asarray([1.0], dtype=np.float32)

            stage_timings_ms["per_frame"] = 0.0
            stage_timings_ms["aggregate"] = 0.0
            stage_timings_ms["total"] = (time.time() - t_start) * 1000.0
            return {
                ...
                "__meta_override__": {
                    ...
                    "feature_set": str(cfg.get("feature_set") or DEFAULT_FEATURE_SET),
        ...
        def _work(pos: int, idx: int) -> Tuple[int, Dict[str, float]]:
            ...
            # Robustness: crop letterbox bars for image-based metrics (Audit v3 improvement).
            fr = _crop_letterbox_rgb(fr)
            out: Dict[str, float] = {}
            if need_balance:
        ...
        stage_timings_ms["per_frame"] = (time.time() - t_stage) * 1000.0
        ...
        stage_timings_ms["aggregate"] = (time.time() - t_stage) * 1000.0
        stage_timings_ms["total"] = (time.time() - t_start) * 1000.0

        # Per-feature presence ratio (helps models distinguish NaN vs real zeros).
        finite = np.isfinite(frame_feature_values)
        frame_feature_present_ratio = finite.mean(axis=0).astype(np.float32)
    ...
    def save_results(
        ...
        """
        Ensure Audit v3 baseline meta fields exist:
        - meta.stage_timings_ms (dict)
        """
        meta = dict(metadata or {})
        st = None
        try:
            st = results.get("_stage_timings_ms")
        except Exception:
            st = None
        if not isinstance(st, dict):
            st = {}
        meta["stage_timings_ms"] = st
        try:
            rp = getattr(self, "_resource_profile_before", None)
            if isinstance(rp, dict) and rp:
                meta["resource_profile_before"] = dict(rp)
        except Exception:
            pass

        clean = dict(results or {})
        clean.pop("_stage_timings_ms", None)
        return super().save_results(
            results=clean, metadata=meta, use_compressed=use_compressed, embeddings_key=embeddings_key
        )
