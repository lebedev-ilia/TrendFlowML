...
_vp = Path(__file__).resolve().parent  # scene_classification/
for _ in range(2):
    _vp = _vp.parent  # -> modules -> VisualProcessor
_vp_s = str(_vp)
if _vp_s not in sys.path:
    sys.path.insert(0, _vp_s)
elif sys.path[0] != _vp_s:
    try:
        sys.path.remove(_vp_s)
    except ValueError:
        pass
    sys.path.insert(0, _vp_s)
...
from modules.scene_classification.utils.scene_classification import Places365SceneClassifier
