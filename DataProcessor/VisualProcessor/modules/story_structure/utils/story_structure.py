...
def _frame_feature_present_ratio(*, curves: List[np.ndarray]) -> np.ndarray:
    """
    Compute per-frame present ratio for model-facing float curves.
    Assumes all curves are shape (N,). Ratio is finite_count / K.
    """
    if not curves:
        return np.asarray([], dtype=np.float32)
    n = int(curves[0].shape[0])
    k = int(len(curves))
    if n <= 0 or k <= 0:
        return np.asarray([], dtype=np.float32)
    m = np.zeros((n, k), dtype=bool)
    for j, c in enumerate(curves):
        cj = np.asarray(c, dtype=np.float32).reshape(-1)
        if int(cj.size) != n:
            raise RuntimeError(f"{MODULE_NAME} | frame_feature_present_ratio: curve size mismatch")
        m[:, j] = np.isfinite(cj)
    return (np.sum(m, axis=1).astype(np.float32) / float(k)).astype(np.float32)


def _suppress_peaks_by_time(
    *,
    peaks_idx: np.ndarray,
    peak_heights: np.ndarray,
    times_s: np.ndarray,
    min_time_sep_sec: float,
) -> np.ndarray:
    """
    Greedy suppression of peaks: keep higher peaks, enforce minimum time separation.
    """
    if peaks_idx.size == 0:
        return peaks_idx.astype(np.int32)
    if min_time_sep_sec <= 0:
        return peaks_idx.astype(np.int32)
    idx = np.asarray(peaks_idx, dtype=np.int32).reshape(-1)
    h = np.asarray(peak_heights, dtype=np.float32).reshape(-1)
    if h.size != idx.size:
        return idx
    ...
    for oi in order.tolist():
        pi = int(idx[int(oi)])
        if pi < 0 or pi >= int(times_s.size):
            continue
        t = float(times_s[pi])
        ok = True
        for tt in kept_t:
            if abs(t - float(tt)) < float(min_time_sep_sec):
                ok = False
                break
        if ok:
            kept.append(pi)
            kept_t.append(t)
    kept_sorted = np.asarray(sorted(set(kept)), dtype=np.int32)
    return kept_sorted
...
class StoryStructureBaselineModule(BaseModule):
    ...
    def process(self, frame_manager: FrameManager, frame_indices: List[int], config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if story_energy_curve.size >= 4:
            ...
            peaks_idx0 = np.asarray(peaks, dtype=np.int32)
            heights0 = np.asarray(props.get("peak_heights", np.asarray([], dtype=np.float32)), dtype=np.float32).reshape(-1)
            min_time_sep_sec = float(config.get("min_energy_peak_sep_sec", 1.0))
            peaks_idx = _suppress_peaks_by_time(peaks_idx=peaks_idx0, peak_heights=heights0, times_s=times_s, min_time_sep_sec=min_time_sep_sec)
        ...
        text_mode = str(config.get("text_mode", "none")).strip().lower()
        ...
        # Audit v3 policy: module remains status=ok if visual deps are present.
        # Missing OCR only disables text-derived curve (topic_shift_curve_present=false) and records reason in meta.
        ocr_reason = None
        if text_mode in ("ocr_clip_text", "ocr"):
            ocr_rows, ocr_reason = self._load_ocr_rows_optional(rs_path=str(self.rs_path))
            if not ocr_rows:
                self.logger.warning("%s | OCR missing/empty: text curve disabled (reason=%s)", MODULE_NAME, str(ocr_reason or "dependency_missing"))
            else:
        ...
        # Scalar features (model-facing tabular)
        features: Dict[str, Any] = {
            ...
            # climax
            "climax_frame_index": int(climax_frame),  # union-frame index
        ...
        feature_values = np.asarray([_as_float_feature(features.get(k)) for k in _FEATURE_NAMES_V1], dtype=np.float32).reshape(-1)
        ...
        # Model-facing per-frame quality metric (helps models interpret NaNs)
        frame_feature_present_ratio = _frame_feature_present_ratio(
            curves=[
                np.asarray(story_energy_curve, dtype=np.float32).reshape(-1),
                np.asarray(mot_s, dtype=np.float32).reshape(-1),
                np.asarray(emb_rate_curve, dtype=np.float32).reshape(-1),
                np.asarray(topic_shift_curve, dtype=np.float32).reshape(-1),
            ]
        )
    ...
    def run(self, frames_dir: str, config: Dict[str, Any], metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        try:
            ...
            # Move stage timings into meta.stage_timings_ms (Audit v3)
            stage_timings_ms = None
            if isinstance(results, dict) and "stage_timings_ms" in results:
                try:
                    stage_timings_ms = results.pop("stage_timings_ms")
                except Exception:
                    stage_timings_ms = None
            ...
            save_metadata = {
                ...
                # Reproducibility config highlights (small + stable)
                "min_frames": int((config or {}).get("min_frames", 30)),
                "max_frames": int((config or {}).get("max_frames", self._max_frames)),
                "energy_smoothing_sigma": float((config or {}).get("energy_smoothing_sigma", 1.0)),
                "text_mode": str((config or {}).get("text_mode", "none")),
                "clip_text_model_spec": str((config or {}).get("clip_text_model_spec", "clip_text_triton")),
                "clip_text_batch_size": int((config or {}).get("clip_text_batch_size", 64)),
                "ocr_max_chars_per_frame": int((config or {}).get("ocr_max_chars_per_frame", 256)),
                "triton_http_url": (config or {}).get("triton_http_url"),
                "min_energy_peak_sep_sec": float((config or {}).get("min_energy_peak_sep_sec", 1.0)),
            }
