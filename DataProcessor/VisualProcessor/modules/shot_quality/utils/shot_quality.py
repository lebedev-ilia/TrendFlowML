...
def _crop_letterbox_rgb(frame_rgb: np.ndarray, *, black_thr: float = 0.03, min_crop_frac: float = 0.02) -> np.ndarray:
    """
    Heuristic removal of top/bottom black bars (letterbox).
    Mirrors logic from frames_composition to stabilize image-based metrics.
    """
    try:
        ...
        thr = float(black_thr)
        top = 0
        while top < h and float(row_mean[top]) < thr:
            top += 1
        bot = h
        while bot > 0 and float(row_mean[bot - 1]) < thr:
            bot -= 1
        min_crop = int(max(1, round(h * float(min_crop_frac))))
        if top >= min_crop or (h - bot) >= min_crop:
            top2 = min(top, h - 1)
            bot2 = max(bot, top2 + 1)
            return frame_rgb[top2:bot2, :, :]
        return frame_rgb
    except Exception:
        return frame_rgb
...
def temporal_flicker(prev_gray: Optional[np.ndarray], gray: np.ndarray) -> float:
    ...
    if prev_gray.shape != gray.shape:
        # Per-frame resize can yield different (h,w) after letterbox crop; align to current.
        prev_gray = cv2.resize(
            prev_gray.astype(np.float32), (gray.shape[1], gray.shape[0]), interpolation=cv2.INTER_LINEAR
        )
...
def rolling_shutter_artifacts_score(prev_frame_bgr: Optional[np.ndarray], curr_frame_bgr: np.ndarray) -> float:
    ...
    if gray_prev.shape != gray_curr.shape:
        ...
        gray_prev = cv2.resize(gray_prev, (w, h), interpolation=cv2.INTER_LINEAR)
...
class ShotQualityModule(BaseModule):
    ...
    def _load_cut_detection_model_facing(self) -> Optional[Dict[str, Any]]:
        """
        Load cut_detection model-facing NPZ (canonical stream for hard cut events).
        Best-effort fallback: return None if not found (legacy runs).
        """
        if not self.rs_path:
            return None
        try:
            base = Path(self.rs_path) / "cut_detection"
            if not base.exists():
                return None
            cands = list(base.glob("cut_detection_model_facing_*.npz"))
            if not cands:
                return None
            p = max(cands, key=lambda x: x.stat().st_mtime)
            return self._load_npz(str(p))
        except Exception:
            return None
    ...
    def process(self, frame_manager: FrameManager, frame_indices: List[int], config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for i, frame_idx in enumerate(frame_indices_np.tolist()):
            ...
            # Robustness: crop letterbox bars for image-based metrics (avoid black bars bias).
            frame_rgb_content = _crop_letterbox_rgb(frame_rgb)

            # Downscale RGB first, then compute BGR/GRAY for metrics
            max_dim = max(int(frame_rgb_content.shape[0]), int(frame_rgb_content.shape[1]))
            if max_dim > ANALYSIS_MAX_DIM:
                scale = float(ANALYSIS_MAX_DIM) / float(max_dim)
                new_w = max(1, int(round(int(frame_rgb_content.shape[1]) * scale)))
                new_h = max(1, int(round(int(frame_rgb_content.shape[0]) * scale)))
                rgb_small = cv2.resize(frame_rgb_content, (new_w, new_h), interpolation=cv2.INTER_AREA)
            else:
                rgb_small = frame_rgb_content
        ...
        # Quality/QA for NaNs: per-feature finite ratio (helps models interpret missing values)
        try:
            finite = np.isfinite(frame_features)
            frame_feature_present_ratio = np.mean(finite, axis=0).astype(np.float32)
        except Exception:
            frame_feature_present_ratio = np.full((int(frame_features.shape[1]),), np.nan, dtype=np.float32)

        # --- Shot segmentation from cut_detection (canonical: model-facing NPZ events) ---
        self.logger.info(f"{MODULE_NAME} | process | Построение shot boundaries из cut_detection_model_facing (preferred)")
        shot_boundaries_source = "cut_detection_model_facing"
        hard_cut_union: List[int] = []
        mf = self._load_cut_detection_model_facing()
        if isinstance(mf, dict):
            try:
                mf_fi = np.asarray(mf.get("frame_indices"), dtype=np.int32).reshape(-1)
                ev_type = np.asarray(mf.get("event_type_id"), dtype=np.int16).reshape(-1)
                ev_pair = np.asarray(mf.get("event_pair_index"), dtype=np.int32).reshape(-1)
                m = (ev_type == 1) & (ev_pair >= 0) & (ev_pair < max(0, int(mf_fi.size) - 1))
                # For a hard cut at pair_index p (between frames p and p+1), transition frame is position (p+1)
                pos = (ev_pair[m].astype(np.int32) + 1).astype(np.int32)
                for pp in np.unique(pos).tolist():
                    if 0 <= int(pp) < int(mf_fi.size):
                        hard_cut_union.append(int(mf_fi[int(pp)]))
            except Exception:
                hard_cut_union = []

        if not hard_cut_union:
            # Compat fallback: legacy dict payload from cut_detection main NPZ.
            shot_boundaries_source = "cut_detection_legacy_detections"
            ...
            boundaries = [int(x) for x in shot_boundaries]
        else:
            # Build boundaries in union-domain, anchored to this module's axis.
            axis_start = int(frame_indices_np[0])
            axis_end = int(frame_indices_np[-1])
            cuts = sorted({int(x) for x in hard_cut_union if axis_start < int(x) <= axis_end})
            boundaries = [axis_start] + cuts + [axis_end]

        # Make strictly increasing and at least 2
        boundaries = [int(x) for x in boundaries]
        boundaries = [boundaries[0]] + [b for i, b in enumerate(boundaries[1:], 1) if int(b) > int(boundaries[i - 1])]
        if len(boundaries) < 2:
            boundaries = [int(frame_indices_np[0]), int(frame_indices_np[-1])]

        shot_start_frames = np.asarray(boundaries[:-1], dtype=np.int32)
        shot_end_frames = np.asarray(boundaries[1:], dtype=np.int32)

        # Assign each frame to shot via bisect over start_frames (union-domain indices)
        import bisect
        ...
        # Persist shot-level quality aggregates as arrays (not only in ui_payload)
        shot_quality_topk_ids = np.asarray(shot_topk_ids, dtype=np.int32).reshape(int(s), int(k))
        shot_quality_topk_probs = np.asarray(shot_topk_probs, dtype=np.float32).reshape(int(s), int(k))
        shot_quality_conf_mean = np.asarray(shot_conf_mean, dtype=np.float32).reshape(int(s),)
        shot_quality_entropy_mean = np.asarray(shot_entropy_mean, dtype=np.float32).reshape(int(s),)
        ...
        meta_out = {
            ...
            "shot_boundaries_source": str(shot_boundaries_source),
        }
        ...
        return {
            ...
            "shot_quality_topk_ids": shot_quality_topk_ids,
            "shot_quality_topk_probs": shot_quality_topk_probs,
            "shot_quality_conf_mean": shot_quality_conf_mean,
            "shot_quality_entropy_mean": shot_quality_entropy_mean,
            "shot_frame_feature_present_ratio": np.stack(
                [
                    (np.mean(np.isfinite(frame_features[shot_ids == sid]), axis=0).astype(np.float32) if np.any(shot_ids == sid) else np.zeros((int(frame_features.shape[1]),), dtype=np.float32))
                    for sid in range(int(s))
                ],
                axis=0,
            ).astype(np.float32),
    ...
    def run(self, frames_dir: str, config: Dict[str, Any], metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        try:
            ...
            # Move impl_meta into meta (debug), avoid top-level object payload in NPZ
            impl_meta = None
            if isinstance(results, dict) and "impl_meta" in results:
                try:
                    impl_meta = results.pop("impl_meta")
                    if isinstance(impl_meta, np.ndarray) and impl_meta.dtype == object:
                        impl_meta = impl_meta.item() if impl_meta.shape == () else impl_meta.tolist()
                except Exception:
                    impl_meta = None

            base_models = super().get_models_used(config or {}, metadata or {})
            clip_models: List[Dict[str, Any]] = []
            if isinstance(impl_meta, dict):
                cm = impl_meta.get("clip_model_name")
            ...
            merged_models: List[Dict[str, Any]] = list(base_models)
            for m in clip_models:
                if not any(str(x.get("model_name") or "") == str(m.get("model_name") or "") for x in merged_models):
