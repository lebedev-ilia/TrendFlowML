#!/bin/bash
# Скрипт для запуска тестов shot_quality компонента на всех видео (последовательно).
...
echo "=========================================="
echo "Запуск тестов shot_quality компонента"
...
echo ""
echo "[0] Тестирование: ${SHORTEST_VIDEO} (test_shot_quality_shortest)"
echo "----------------------------------------"
...
LOG_FILE="/tmp/shot_quality_test_test_shot_quality_shortest.log"
if "${PYTHON}" "${MAIN_SCRIPT}" \
    --video-path "${video_path}" \
    --global-config "${GLOBAL_CONFIG}" \
    --profile-path "${CONFIG_PROFILE}" \
    --platform-id youtube \
    --video-id test_shot_quality_shortest \
    --run-id test_shot_quality_shortest \
    --output-dir "${RESULTS_DIR}" \
    > "${LOG_FILE}" 2>&1; then
    echo "✅ Успешно: test_shot_quality_shortest"
else
    echo "❌ Ошибка (exit code: $?): test_shot_quality_shortest"
...
for i in "${!VIDEOS[@]}"; do
    video="${VIDEOS[$i]}"
    video_id="test_shot_quality_$((i+2))"
