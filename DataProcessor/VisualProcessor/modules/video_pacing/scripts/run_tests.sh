#!/bin/bash
# Скрипт для запуска тестов video_pacing компонента на всех видео (последовательно).
...
CONFIG_PROFILE="${CONFIG_PROFILE:-"${BASE_DIR}/DataProcessor/configs/audit_v3/visual/profile_video_pacing.yaml"}"
...
echo "=========================================="
echo "Запуск тестов video_pacing компонента"
...
echo ""
echo "[0] Тестирование: ${SHORTEST_VIDEO} (test_video_pacing_shortest)"
echo "----------------------------------------"
...
LOG_FILE="/tmp/video_pacing_test_test_video_pacing_shortest.log"
if "${PYTHON}" "${MAIN_SCRIPT}" \
    --video-path "${video_path}" \
    --global-config "${GLOBAL_CONFIG}" \
    --profile-path "${CONFIG_PROFILE}" \
    --platform-id youtube \
    --video-id test_video_pacing_shortest \
    --run-id test_video_pacing_shortest \
    --output-dir "${RESULTS_DIR}" \
    > "${LOG_FILE}" 2>&1; then
    echo "✅ Успешно: test_video_pacing_shortest"
else
    echo "❌ Ошибка (exit code: $?): test_video_pacing_shortest"
...
for i in "${!VIDEOS[@]}"; do
    video="${VIDEOS[$i]}"
    video_id="test_video_pacing_$((i+2))"
    ...
    LOG_FILE="/tmp/video_pacing_test_${video_id}.log"
    if "${PYTHON}" "${MAIN_SCRIPT}" \
        --video-path "${video_path}" \
        --global-config "${GLOBAL_CONFIG}" \
        --profile-path "${CONFIG_PROFILE}" \
