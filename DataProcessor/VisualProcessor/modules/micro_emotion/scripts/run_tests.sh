#!/bin/bash
# Скрипт для запуска тестов micro_emotion компонента на всех видео (последовательно).
...
CONFIG_PROFILE="${CONFIG_PROFILE:-"${BASE_DIR}/DataProcessor/configs/audit_v3/visual/profile_micro_emotion.yaml"}"
...
echo "=========================================="
echo "Запуск тестов micro_emotion компонента"
...
echo ""
echo "[0] Тестирование: ${SHORTEST_VIDEO} (test_micro_emotion_shortest)"
echo "----------------------------------------"
...
LOG_FILE="/tmp/micro_emotion_test_test_micro_emotion_shortest.log"
if "${PYTHON}" "${MAIN_SCRIPT}" \
    --video-path "${video_path}" \
    --global-config "${GLOBAL_CONFIG}" \
    --profile-path "${CONFIG_PROFILE}" \
    --platform-id youtube \
    --video-id test_micro_emotion_shortest \
    --run-id test_micro_emotion_shortest \
    --output-dir "${RESULTS_DIR}" \
    > "${LOG_FILE}" 2>&1; then
    echo "✅ Успешно: test_micro_emotion_shortest"
else
    echo "❌ Ошибка (exit code: $?): test_micro_emotion_shortest"
...
for i in "${!VIDEOS[@]}"; do
    video="${VIDEOS[$i]}"
    video_id="test_micro_emotion_$((i+2))"
    ...
    LOG_FILE="/tmp/micro_emotion_test_${video_id}.log"
    if "${PYTHON}" "${MAIN_SCRIPT}" \
        --video-path "${video_path}" \
        --global-config "${GLOBAL_CONFIG}" \
        --profile-path "${CONFIG_PROFILE}" \
