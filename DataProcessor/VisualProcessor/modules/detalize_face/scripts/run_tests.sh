#!/bin/bash
# Скрипт для запуска тестов detalize_face компонента на всех видео
...
CONFIG_PROFILE="${CONFIG_PROFILE:-"${BASE_DIR}/DataProcessor/configs/audit_v3/visual/profile_detalize_face.yaml"}"
...
echo "=========================================="
echo "Запуск тестов detalize_face компонента"
echo "Всего видео: ${#VIDEOS[@]}"
echo "=========================================="

for i in "${!VIDEOS[@]}"; do
    video="${VIDEOS[$i]}"
    video_id="test_detalize_face_$((i+2))"
    ...
    "${PYTHON}" "${MAIN_SCRIPT}" \
        ...
        2>&1 | tee "/tmp/detalize_face_test_${video_id}.log" | tail -20
    ...
    if [ ${exit_code} -eq 0 ]; then
        ...
        echo "Лог сохранен: /tmp/detalize_face_test_${video_id}.log"
    fi
done
