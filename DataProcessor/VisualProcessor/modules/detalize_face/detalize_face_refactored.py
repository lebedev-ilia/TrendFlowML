...
from _utils.compact_features import extract_per_face_aggregates
...
SCHEMA_VERSION = "detalize_face_npz_v1"
...
class DetalizeFaceModule(BaseModule):
    ...
    def process(
        ...
        Args:
            ...
            frame_indices: Список индексов кадров для обработки (уже отфильтрованы по frames_with_face)
            config: Конфигурация модуля (не используется, параметры заданы в __init__)
            
        Returns:
            Baseline NPZ payload (model-facing, no raw per-frame dicts):
            - frame_indices, times_s
            - face_count + primary_* time-series
            - faces_agg (per-track aggregates)
            - summary
        """
        # IMPORTANT: frame_indices passed here should already be filtered to only include
        # frames where core_face_landmarks found faces. No additional filtering needed.
        # This ensures we process ALL frames with faces, not just those selected by Segmenter.
        if not frame_indices:
            return self._empty_result(0)
        
        # Обновляем frames_with_face в extractor для обработки (используем переданные индексы)
        original_frames_with_face = self.extractor.frames_with_face
        self.extractor.frames_with_face = sorted([int(x) for x in frame_indices])
    ...
    def _format_results_for_npz(
        ...
        """
        # Deterministic per-frame arrays (model-facing). No raw "frames" dict in baseline.
        n = int(len(frame_indices))
        face_count = np.full((n,), np.nan, dtype=np.float32)
        ...
        summary = {
            ...
            "frames_with_faces": int(np.sum(np.isfinite(face_count) & (face_count > 0))),
    ...
    def _empty_result(self, total_frames: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "primary_gaze_at_camera_prob": np.asarray([], dtype=np.float32),
            "primary_blink_rate": np.asarray([], dtype=np.float32),
            "primary_attention_score": np.asarray([], dtype=np.float32),
            "primary_quality_proxy_score": np.asarray([], dtype=np.float32),
            "primary_face_sharpness": np.asarray([], dtype=np.float32),
            "primary_occlusion_proxy": np.asarray([], dtype=np.float32),
            "primary_speech_activity_prob": np.asarray([], dtype=np.float32),
            "faces_agg": {},
        }
    ...
    def _build_ui_payload(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        payload = {
            ...
            "curves": {
                "face_count": {"npz_key": "face_count", "label": "faces per frame"},
                "primary_gaze_at_camera_prob": {"npz_key": "primary_gaze_at_camera_prob", "label": "gaze at camera"},
                "primary_blink_rate": {"npz_key": "primary_blink_rate", "label": "blink rate"},
                "primary_attention_score": {"npz_key": "primary_attention_score", "label": "attention score"},
                "primary_quality_proxy_score": {"npz_key": "primary_quality_proxy_score", "label": "quality proxy"},
                "primary_face_sharpness": {"npz_key": "primary_face_sharpness", "label": "sharpness"},
                "primary_occlusion_proxy": {"npz_key": "primary_occlusion_proxy", "label": "occlusion proxy"},
                "primary_speech_activity_prob": {"npz_key": "primary_speech_activity_prob", "label": "speech activity"},
            },
    ...
    def run(
        ...
        IMPORTANT: This module processes ALL frames where core_face_landmarks found faces,
        ignoring Segmenter's sampling for this module. This ensures complete coverage of
        all frames with detected faces.
        """
        if metadata is None:
        ...
        try:
            ...
            # IMPORTANT: This module processes ONLY frames where core_face_landmarks found faces.
            # We ignore Segmenter's sampling for this module and use frames_with_face directly.
            if not self.extractor.frames_with_face:
                # No faces found by core_face_landmarks -> empty result
                self.logger.warning(f"{self.module_name} | no frames with faces from core_face_landmarks")
                results = self._empty_result(int(metadata.get("total_frames") or 0))
            else:
                # Use all frames where core_face_landmarks found faces (ignore Segmenter sampling)
                frames_to_process = sorted([int(x) for x in self.extractor.frames_with_face])
            ...
            # Обновляем total_frames из metadata (summary хранит контекст всего видео)
            if isinstance(results, dict) and isinstance(results.get("summary"), dict):
                results["summary"]["total_frames"] = int(metadata.get("total_frames") or 0)
            ...
            is_empty = results.get("summary", {}).get("frames_with_faces", 0) == 0
            if is_empty:
