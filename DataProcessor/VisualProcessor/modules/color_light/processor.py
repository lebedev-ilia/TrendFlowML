...
class ColorLightProcessor(BaseModule):
    ...
    VERSION = "1.1"
    SCHEMA_VERSION = "color_light_npz_v1"
    ...
    def process(
        ...
        for i, (_, scene_id, scene) in enumerate(sorted(scene_items, key=lambda x: x[0])):
            if not isinstance(scene, dict) or "indices" not in scene:
                continue
            indices = list(scene.get("indices") or [])
            if not indices:
            ...
            scene_frame_indices = np.unique(np.asarray(scene_frame_indices, dtype=np.int32))

            all_frame_features[scene_key] = {}
            scene_frame_features = []

            # ==== FEATURE EXTRACTION ====
            for k, frame_idx in enumerate(scene_frame_indices):
                ...
                logger.info(
                    f"Сцена {i+1}/{len(scenes)} | Кадр {k+1}/{len(scene_frame_indices)} обработан"
                )
                
                # Baseline contract: granular progress (>=10 updates per run)
                # Обновляем прогресс каждые ~6.7% (15 обновлений за run)
                total_frames_to_process = total_scenes * len(scene_frame_indices) if total_scenes > 0 and len(scene_frame_indices) > 0 else max(1, len(frame_indices))
                current_progress = i * len(scene_frame_indices) + k
                progress_interval = max(1, total_frames_to_process // 15)  # 15 обновлений минимум
                if current_progress % progress_interval == 0 or current_progress == total_frames_to_process - 1:
                    meta = getattr(frame_manager, "meta", {}) or {}
                    self._append_state_event_if_possible(
                        ...
                        event={
                            ...
                            "component": self.module_name,
                            "status": "running",
                            "progress": float(current_progress) / max(1, total_frames_to_process),
                            "done": int(current_progress),
    ...
    def save_results(
        ...
        # Baseline contract: stage_timings_ms in summary
        if stage_timings_ms:
