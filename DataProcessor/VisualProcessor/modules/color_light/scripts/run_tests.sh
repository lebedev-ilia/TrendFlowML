#!/bin/bash
# Скрипт для запуска тестов color_light компонента на всех видео
...
CONFIG_PROFILE="${CONFIG_PROFILE:-"${BASE_DIR}/DataProcessor/configs/audit_v3/visual/profile_color_light.yaml"}"
...
echo "=========================================="
echo "Запуск тестов color_light компонента"
echo "Всего видео: ${#VIDEOS[@]}"
echo "=========================================="

for i in "${!VIDEOS[@]}"; do
    video="${VIDEOS[$i]}"
    video_id="test_color_light_$((i+2))"
    ...
    "${PYTHON}" "${MAIN_SCRIPT}" \
        ...
        2>&1 | tee "/tmp/color_light_test_${video_id}.log" | tail -20
    ...
    if [ ${exit_code} -eq 0 ]; then
        ...
        echo "Лог сохранен: /tmp/color_light_test_${video_id}.log"
    fi
done
