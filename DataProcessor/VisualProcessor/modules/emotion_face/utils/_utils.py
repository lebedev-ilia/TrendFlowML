...
def detect_keyframes(emotion_curve, EMOTION_CLASSES, threshold=0.3, smooth_window=5, 
    ...
    # Проверяем, что массивы не пустые перед обработкой
    valence_arr = np.array(emotion_curve['valence'])
    arousal_arr = np.array(emotion_curve['arousal'])
    
    if valence_arr.size == 0 or arousal_arr.size == 0:
        return {}
