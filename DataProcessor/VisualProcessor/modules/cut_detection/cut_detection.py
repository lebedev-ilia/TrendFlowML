...
class CutDetectionPipeline(BaseModule):
    ...
    def required_dependencies(self) -> List[str]:
        ...
        - core_face_landmarks / core_object_detections are kept as hard deps for jump-cuts related heuristics.
        """
        return ["core_optical_flow", "core_face_landmarks", "core_object_detections"]
    ...
    def _process_video_frames(self, frame_manager, frame_indices, audio_path=None):
        ...
        try:
            ...
            error_msg = str(e)
            logger.error(f"cut_detection | Jump cut detection error: {error_msg}", exc_info=True)
            if "core_face_landmarks" in error_msg or "core_object_detections" in error_msg:
                logger.warning(f"Jump cut detection skipped: {e}")
                jump_idxs = []
                jump_scores = []
            else:
