...
def render_cut_detection(npz_data: Dict[str, Any], meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if times_s is not None and len(times_s) > 0:
        # Hard cuts
        hard_cuts = detections.get("hard_cuts", [])
        if isinstance(hard_cuts, np.ndarray):
            hard_cuts = hard_cuts.tolist()
        hard_cuts_set = set(int(x) for x in hard_cuts if isinstance(x, (int, float, np.integer, np.floating)))
        ...
        # Motion cuts
        motion_cuts = detections.get("motion_cuts", [])
        if isinstance(motion_cuts, np.ndarray):
            motion_cuts = motion_cuts.tolist()
        motion_cuts_set = set(int(x) for x in motion_cuts if isinstance(x, (int, float, np.integer, np.floating)))
        
        # Jump cuts
        jump_cuts = detections.get("jump_cuts", [])
        if isinstance(jump_cuts, np.ndarray):
            jump_cuts = jump_cuts.tolist()
        jump_cuts_set = set(int(x) for x in jump_cuts if isinstance(x, (int, float, np.integer, np.floating)))
...
<head>
    ...
    <title>Cut Detection Debug Render</title>
...
<body>
    <div class="container">
        <h1>Cut Detection Debug Render</h1>
        ...
        <div class="chart-container">
            <h2>Timeline: Cuts Over Time</h2>
            <canvas id="timelineChart"></canvas>
        </div>
        ''' if timeline_js else '<p>No timeline data available</p>'}
