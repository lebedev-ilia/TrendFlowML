...
def _load_core_optical_flow(module: BaseModule) -> Dict[str, Any]:
    ...
    # Audit v3 (deepening algorithms): require compact stats from core_optical_flow_npz_v3
    required_v3 = [
    ...
    missing = [k for k in required_v3 if core.get(k) is None]
    if missing:
        raise RuntimeError(
            "optical_flow | core_optical_flow artifact is missing required v3 keys: "
            + ", ".join(missing)
        )
...
def _extract_npz_meta_models_used(npz_path: str) -> List[Dict[str, Any]]:
    try:
        if not npz_path:
            return []
...
class OpticalFlowModule(BaseModule):
    ...
    def process(self, frame_manager: FrameManager, frame_indices: List[int], config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        core_dt = np.asarray(core["dt_seconds"], dtype=np.float32).reshape(-1)
        core_times = np.asarray(core["times_s"], dtype=np.float32).reshape(-1)

        # v3 compact stats from core_optical_flow (all (N,))
        v3_keys = [
            "flow_mag_std_per_sec_norm",
            "flow_mag_p95_per_sec_norm",
            "flow_dx_mean_per_sec_norm",
        ...
        core_v3 = {k: np.asarray(core[k], dtype=np.float32).reshape(-1) for k in v3_keys}
        if not bool(np.all(core_idx.reshape(-1) == np.asarray(core.get("frame_indices")).reshape(-1))):
        ...
        dt_aligned = np.full((int(want.size),), np.nan, dtype=np.float32)
        times_aligned = np.full((int(want.size),), np.nan, dtype=np.float32)
        frame_feat = {k: np.full((int(want.size),), np.nan, dtype=np.float32) for k in v3_keys}
        missing_count = 0
        for j, fi in enumerate(want.tolist()):
            ...
            if p < 0:
                ...
                dt_aligned[j] = float(core_dt[int(p)]) if int(p) < int(core_dt.size) else float("nan")
                times_aligned[j] = float(core_times[int(p)]) if int(p) < int(core_times.size) else float("nan")
                for k in v3_keys:
                    a = core_v3[k]
                    frame_feat[k][j] = float(a[int(p)]) if int(p) < int(a.size) else float("nan")
        ...
        cam_shake = frame_feat["cam_shake_std_norm"]
        cam_rot = frame_feat["cam_affine_rotation"]
        cam_tx = frame_feat["cam_tx_per_sec_norm"]
        cam_ty = frame_feat["cam_ty_per_sec_norm"]
        trans_abs = np.sqrt(np.square(cam_tx) + np.square(cam_ty)).astype(np.float32)
        feat_vals = np.asarray(
            [
                float(np.nanmean(curve_for_stats)) if curve_for_stats.size else float("nan"),
                float(np.nanmedian(curve_for_stats)) if curve_for_stats.size else float("nan"),
                float(np.nanpercentile(curve_for_stats, 90)) if curve_for_stats.size else float("nan"),
                float(np.nanvar(curve_for_stats)) if curve_for_stats.size else float("nan"),
                float(missing_count) / float(max(1, int(want.size))),
                float(np.nanmean(cam_shake[1:])) if cam_shake.size >= 2 else float("nan"),
                float(np.nanmean(np.abs(cam_rot[1:]))) if cam_rot.size >= 2 else float("nan"),
                float(np.nanmean(trans_abs[1:])) if trans_abs.size >= 2 else float("nan"),
                float(np.nanmean(frame_feat["flow_consistency"][1:])) if frame_feat["flow_consistency"].size >= 2 else float("nan"),
            ],
            dtype=np.float32,
        ).reshape(-1)
        ...
        # Build dense per-frame compact matrix for models
        frame_feature_names = np.asarray(
            [
                "motion_norm_per_sec_mean",
                "flow_mag_std_per_sec_norm",
                "flow_mag_p95_per_sec_norm",
        ...
        cols = [
            curve,
            frame_feat["flow_mag_std_per_sec_norm"],
            frame_feat["flow_mag_p95_per_sec_norm"],
            frame_feat["flow_dx_mean_per_sec_norm"],
            frame_feat["flow_dy_mean_per_sec_norm"],
            frame_feat["flow_dir_sin_mean"],
            frame_feat["flow_dir_cos_mean"],
            frame_feat["flow_dir_dispersion"],
            frame_feat["flow_div_abs_mean"],
            frame_feat["flow_consistency"],
            frame_feat["cam_affine_scale"],
            frame_feat["cam_affine_rotation"],
            frame_feat["cam_tx_per_sec_norm"],
            frame_feat["cam_ty_per_sec_norm"],
            frame_feat["cam_shake_std_norm"],
            frame_feat["bg_ratio"],
        ]
    ...
    def run(self, frames_dir: str, config: Dict[str, Any], metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        try:
            ...
            # stage timings in meta (Audit v3 baseline contract)
            stage_timings_ms = {
            ...
            # models_used: consumer-only; best-effort propagate from core_optical_flow meta for provenance.
            core_path = str(Path(self.rs_path) / "core_optical_flow" / "flow.npz") if self.rs_path else ""
            save_metadata["models_used"] = _extract_npz_meta_models_used(core_path)
