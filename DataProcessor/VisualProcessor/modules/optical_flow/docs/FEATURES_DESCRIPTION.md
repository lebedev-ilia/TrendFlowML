...
- `optical_flow` формирует **baseline-артефакт** `optical_flow/optical_flow.npz` со `schema_version="optical_flow_npz_v3"`.
...
- **Per-frame compact (model-facing)**: `frame_feature_values (N,D)` + `frame_feature_names (D,)` (camera motion + direction/consistency)
- **Aggregates (tabular)**: `feature_names (F,)` + `feature_values (F,)` с устойчивыми статистиками кривой (mean/median/p90/var + missing_frame_ratio)
...
Структура артефакта v3, QA/melt, валидатор: **`FEATURE_DESCRIPTION.md`**.
