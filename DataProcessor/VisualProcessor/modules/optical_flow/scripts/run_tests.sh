#!/bin/bash
# Скрипт для запуска тестов optical_flow компонента на всех видео (последовательно).
...
CONFIG_PROFILE="${CONFIG_PROFILE:-"${BASE_DIR}/DataProcessor/configs/audit_v3/visual/profile_optical_flow.yaml"}"
...
echo "=========================================="
echo "Запуск тестов optical_flow компонента"
...
echo ""
echo "[0] Тестирование: ${SHORTEST_VIDEO} (test_optical_flow_shortest)"
echo "----------------------------------------"
...
LOG_FILE="/tmp/optical_flow_test_test_optical_flow_shortest.log"
if "${PYTHON}" "${MAIN_SCRIPT}" \
    --video-path "${video_path}" \
    --global-config "${GLOBAL_CONFIG}" \
    --profile-path "${CONFIG_PROFILE}" \
    --platform-id youtube \
    --video-id test_optical_flow_shortest \
    --run-id test_optical_flow_shortest \
    --output-dir "${RESULTS_DIR}" \
    > "${LOG_FILE}" 2>&1; then
    echo "✅ Успешно: test_optical_flow_shortest"
else
    echo "❌ Ошибка (exit code: $?): test_optical_flow_shortest"
...
for i in "${!VIDEOS[@]}"; do
    video="${VIDEOS[$i]}"
    video_id="test_optical_flow_$((i+2))"
    ...
    LOG_FILE="/tmp/optical_flow_test_${video_id}.log"
    if "${PYTHON}" "${MAIN_SCRIPT}" \
        --video-path "${video_path}" \
        --global-config "${GLOBAL_CONFIG}" \
        --profile-path "${CONFIG_PROFILE}" \
