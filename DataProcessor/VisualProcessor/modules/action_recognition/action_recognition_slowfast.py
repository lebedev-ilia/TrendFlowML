...
class SlowFastActionRecognizer(BaseModule):
    ...
    VERSION = "1.0"
    SCHEMA_VERSION = "action_recognition_npz_v1"
    ...
    def _extract_embeddings(self, clips: List[List[np.ndarray]]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for start in range(0, total_clips, self.batch_size):
            ...
            raw_all.append(feat.cpu().numpy().astype(np.float32))
    ...
    def _aggregate(self, normed_embeddings: np.ndarray, raw_embeddings: np.ndarray) -> Dict[str, Any]:
        ...
        if n == 0:
            ...
            return {
                "mean_embedding_norm_raw": float("nan"),
                "std_embedding_norm_raw": float("nan"),
                "mean_embedding_norm": float("nan"),
                "std_embedding_norm": float("nan"),
        ...
        raw_norms = np.linalg.norm(raw_embeddings, axis=1) if len(raw_embeddings) else np.array([], dtype=np.float32)
        mean_norm_raw = float(np.mean(raw_norms)) if raw_norms.size else float("nan")
        std_norm_raw = float(np.std(raw_norms)) if raw_norms.size else float("nan")
        # backward-compatible aliases (keep names but point to raw norms)
        mean_norm = mean_norm_raw
        std_norm = std_norm_raw
    ...
    def _prepare_tracks(
        ...
        # Фильтруем кадры с person детекциями (class_id=0)
        PERSON_CLASS_ID = 0
        frame_indices_set = set(frame_indices)
        frames_with_person: List[int] = []
        ...
        for i, frame_idx_global in enumerate(det_frame_indices):
            ...
            if has_person:
                frames_with_person.append(int(frame_idx_global))
        ...
        # Простая группировка: разбиваем на сегменты по последовательным кадрам
        frames_with_person_sorted = sorted(set(frames_with_person))
        
        # Разбиваем на сегменты: если разрыв между кадрами > 10, начинаем новый сегмент
        segments: Dict[int, List[int]] = {}
        current_segment_id = 0
        current_segment: List[int] = []
        prev_frame = None
        
        for frame_idx in frames_with_person_sorted:
            if prev_frame is None:
                current_segment.append(frame_idx)
            elif frame_idx - prev_frame <= 10:  # Последовательные кадры или небольшой разрыв
                current_segment.append(frame_idx)
