#!/bin/bash
# Скрипт для запуска тестов action_recognition компонента на всех видео (последовательно).
...
CONFIG_PROFILE="${CONFIG_PROFILE:-"${BASE_DIR}/DataProcessor/configs/audit_v3/visual/profile_action_recognition.yaml"}"
...
echo "=========================================="
echo "Запуск тестов action_recognition компонента"
...
echo ""
echo "[0] Тестирование: ${SHORTEST_VIDEO} (test_action_recognition_shortest)"
echo "----------------------------------------"
...
LOG_FILE="/tmp/action_recognition_test_test_action_recognition_shortest.log"
if "${PYTHON}" "${MAIN_SCRIPT}" \
    --video-path "${video_path}" \
    --global-config "${GLOBAL_CONFIG}" \
    --profile-path "${CONFIG_PROFILE}" \
    --platform-id youtube \
    --video-id test_action_recognition_shortest \
    --run-id test_action_recognition_shortest \
    --output-dir "${RESULTS_DIR}" \
    > "${LOG_FILE}" 2>&1; then
    echo "✅ Успешно: test_action_recognition_shortest"
else
    echo "❌ Ошибка (exit code: $?): test_action_recognition_shortest"
...
for i in "${!VIDEOS[@]}"; do
    video="${VIDEOS[$i]}"
    video_id="test_action_recognition_$((i+2))"
    ...
    LOG_FILE="/tmp/action_recognition_test_${video_id}.log"
    if "${PYTHON}" "${MAIN_SCRIPT}" \
        --video-path "${video_path}" \
        --global-config "${GLOBAL_CONFIG}" \
        --profile-path "${CONFIG_PROFILE}" \
