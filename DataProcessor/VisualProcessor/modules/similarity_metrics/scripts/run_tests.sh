#!/bin/bash
# Скрипт для запуска тестов similarity_metrics компонента на всех видео (последовательно).
...
CONFIG_PROFILE="${CONFIG_PROFILE:-"${BASE_DIR}/DataProcessor/configs/audit_v3/visual/profile_similarity_metrics.yaml"}"
...
echo "=========================================="
echo "Запуск тестов similarity_metrics компонента"
...
echo ""
echo "[0] Тестирование: ${SHORTEST_VIDEO} (test_similarity_metrics_shortest)"
echo "----------------------------------------"
...
LOG_FILE="/tmp/similarity_metrics_test_test_similarity_metrics_shortest.log"
if "${PYTHON}" "${MAIN_SCRIPT}" \
    --video-path "${video_path}" \
    --global-config "${GLOBAL_CONFIG}" \
    --profile-path "${CONFIG_PROFILE}" \
    --platform-id youtube \
    --video-id test_similarity_metrics_shortest \
    --run-id test_similarity_metrics_shortest \
    --output-dir "${RESULTS_DIR}" \
    > "${LOG_FILE}" 2>&1; then
    echo "✅ Успешно: test_similarity_metrics_shortest"
else
    echo "❌ Ошибка (exit code: $?): test_similarity_metrics_shortest"
...
for i in "${!VIDEOS[@]}"; do
    video="${VIDEOS[$i]}"
    video_id="test_similarity_metrics_$((i+2))"
    ...
    LOG_FILE="/tmp/similarity_metrics_test_${video_id}.log"
    if "${PYTHON}" "${MAIN_SCRIPT}" \
        --video-path "${video_path}" \
        --global-config "${GLOBAL_CONFIG}" \
        --profile-path "${CONFIG_PROFILE}" \
