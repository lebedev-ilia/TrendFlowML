# Описание фичей модуля similarity_metrics (baseline v1)
...
- `audio_clap` — **обязателен** (если нет аудио → `error`)
- `text` — optional (нет текста допустимо)
- `pacing` — optional
- `quality` — optional
- `emotion` — optional (нет лиц допустимо)

Также сохраняются агрегаты `reference_similarity_*` в `features` (mean_topn/max/p10 по CLIP).
