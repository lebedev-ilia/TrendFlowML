...
VERSION = "1.0"
SCHEMA_VERSION = "similarity_metrics_npz_v1"
...
class SimilarityBaselineModule(BaseModule):
    ...
    def required_dependencies(self) -> List[str]:
        # Audio is required by policy: no audio -> error.
        return ["core_clip", "clap_extractor"]
    ...
    def process(self, frame_manager: FrameManager, frame_indices: List[int], config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Required audio (CLAP) - no audio -> error
        clap_npz = _load_npz_required(str(self.rs_path), "clap_extractor", fixed_name="clap_extractor_features.npz")
        clap_emb = clap_npz.get("embedding")
        if clap_emb is None:
            raise RuntimeError(f"{MODULE_NAME} | clap_extractor missing `embedding` key")
        ...
        stage_ms = {"process_ms": float((t1 - t0) * 1000.0)}
        ...
        return {
            ...
            "summary": {"stage_timings_ms": stage_ms},
            "ui_payload": {
