"""
Audit v3 renderer for content_domain (offline, NO CDN).
"""
...
def _parse_label_space(semantic_label_names: Optional[np.ndarray]) -> Dict[int, str]:
    id_to_name: Dict[int, str] = {}
    if semantic_label_names is None:
        return id_to_name
...
def render_content_domain(
    ...
    id_to_name = _parse_label_space(semantic_label_names)
    ...
    K = int(frame_topk_ids.shape[1]) if isinstance(frame_topk_ids, np.ndarray) and frame_topk_ids.ndim == 2 else None
            ...
            render["timeline"].append(
                {
                    ...
                    "top1_id": int(lid),
                    "top1_name": str(id_to_name.get(lid, "")) if lid >= 0 else "",
                    "top1_score": None if not np.isfinite(sc) else float(sc),
    ...
    if track_topk_scores is not None and track_topk_scores.size > 0:
        render["distributions"]["video_topk_scores"] = _compute_score_stats(track_topk_scores.reshape(-1))

    # Top domains (by confident top1 frames)
    if frame_topk_ids is not None and frame_is_confident_top1 is not None:
        counts: Dict[int, int] = {}
        scores_sum: Dict[int, float] = {}
        for i in range(int(frame_topk_ids.shape[0])):
            if not bool(frame_is_confident_top1[i]):
                continue
            lid = int(frame_topk_ids[i, 0])
            sc = float(frame_topk_scores[i, 0]) if frame_topk_scores is not None else float("nan")
            if lid < 0 or not np.isfinite(sc):
                continue
            counts[lid] = int(counts.get(lid, 0) + 1)
            scores_sum[lid] = float(scores_sum.get(lid, 0.0) + float(sc))

        top = sorted(counts.items(), key=lambda x: int(x[1]), reverse=True)[:15]
        render["top_domains"] = [
            {
                "domain_id": int(lid),
                "domain_name": str(id_to_name.get(int(lid), "")),
                "confident_frames": int(cnt),
                "avg_score": float(scores_sum.get(int(lid), 0.0) / max(1, int(cnt))),
            }
            for lid, cnt in top
        ]

    # Video aggregate (track=1)
    if track_topk_ids is not None and track_topk_scores is not None and track_topk_ids.size > 0:
        agg = []
        for k in range(int(track_topk_ids.shape[1])):
            lid = int(track_topk_ids[0, k])
            sc = float(track_topk_scores[0, k])
            if lid < 0 or not np.isfinite(sc):
                continue
            agg.append({"id": lid, "name": id_to_name.get(lid, ""), "score": float(sc)})
        render["summary"]["video_aggregate_topk"] = agg
        if track_is_confident_top1 is not None and track_is_confident_top1.size:
            render["summary"]["video_is_confident_top1"] = bool(track_is_confident_top1[0])

    return render


def render_content_domain_html(
    npz_path: str,
    output_path: str,
    *,
    ...
    if os.path.exists(ctx_path):
        ...
        ctx = render_content_domain(npz_data, meta, {})

    summary = ctx.get("summary") or {}
    timeline = ctx.get("timeline") or []
    top_domains = ctx.get("top_domains") or []
    dists = ctx.get("distributions") or {}

    # SVG timeline of top1 score
    ...
    def _stat_row(title: str, st: Dict[str, Any]) -> str:
        if not st:
            return ""
        return (
            f"<tr><td>{_esc(title)}</td>"
            f"<td>{_esc(st.get('min'))}</td><td>{_esc(st.get('max'))}</td>"
            f"<td>{_esc(st.get('mean'))}</td><td>{_esc(st.get('std'))}</td>"
            f"<td>{_esc(st.get('median'))}</td><td>{_esc(st.get('count'))}</td></tr>"
        )
...
<head>
  ...
  <title>content_domain (offline render)</title>
  <style>
...
<body>
  <div class="wrap">
    <h1>content_domain — offline mini-dashboard</h1>
    <div class="card">
      <div class="grid">
    ...
    <div class="card">
      <h2>Top domains (by confident top-1 frames)</h2>
      <table>
        <thead><tr><th>domain</th><th>confident_frames</th><th>avg_score</th></tr></thead>
        <tbody>
          {"".join([f"<tr><td>{_esc(r.get('domain_name'))}</td><td>{_esc(r.get('confident_frames'))}</td><td>{_esc(r.get('avg_score'))}</td></tr>" for r in top_domains])}
        </tbody>
      </table>
    </div>
    ...
    <div class="card">
      ...
      <table>
        <thead><tr><th>scope</th><th>min</th><th>max</th><th>mean</th><th>std</th><th>median</th><th>count</th></tr></thead>
        <tbody>
          {_stat_row("frame_top1_scores", dists.get("frame_top1_scores") or {})}
          {_stat_row("frame_topk_scores", dists.get("frame_topk_scores") or {})}
          {_stat_row("video_topk_scores", dists.get("video_topk_scores") or {})}
        </tbody>
      </table>
    </div>
  </div>
...
"""
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(html)
    logger.info("content_domain | html render saved: %s", output_path)
    return output_path
