...
1. **Настройте пути в скрипте** `add_brand.py`:
   ```python
   videos = "/path/to/your/videos"  # Директория с видео
   photos = "/path/to/your/photos"  # Директория с фото (опционально)
   ```
...
2. **Интерактивная разметка**:
   ...
   - Специальные команды:
     - Пустая строка → пропустить этот логотип
     - `q` → выйти из скрипта
...
1. **Для каждого бренда** в `known_brands/<brand_name>/`:
...
2. **Добавление в Embedding Service**:
   - Категория: `brand`
   - Название: `<brand_name>` (из имени папки)
   - Эмбеддинг: усреднённый и нормализованный
   - Метаданные: `{"source": "known_brands", "num_images": N}`
