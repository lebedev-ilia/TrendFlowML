"""
Renderer for place_semantics (core_identity): render-context JSON + fully-offline HTML (no CDN).

Audit v3:
- HTML must work offline.
- Provide QA-friendly summary of embeddings + top labels.
"""
...
def render_place_semantics(npz_data: Dict[str, Any], meta: Dict[str, Any]) -> Dict[str, Any]:
    # Common keys across identity components.
    times_s = _as_array(npz_data.get("times_s"), dtype=np.float32)
    frame_indices = _as_array(npz_data.get("frame_indices"), dtype=np.int32)
    emb = _as_array(npz_data.get("embeddings"), dtype=np.float32)

    # Some variants store `results_json` as object; we show only counts.
...
def render_place_semantics_html(npz_path: str, output_path: str) -> str:
    ...
    ctx = render_place_semantics(npz_data, meta)
    ...
    if isinstance(norms, np.ndarray) and norms.size > 0:
        chart = _svg_line_chart(times_s=times_s[: norms.size], values=norms.reshape(-1), title="Embedding norm", stroke="#2563eb")
...
<head>
  ...
  <title>place_semantics — render</title>
  <style>
...
<body>
  <div class="wrap">
    <div class="card">
      <div style="font-size:18px; font-weight:700;">place_semantics</div>
    ...
    <div class="card">
      ...
      {chart or "<div class='muted'>No curve data</div>"}
    </div>
  </div>
</body>
</html>
