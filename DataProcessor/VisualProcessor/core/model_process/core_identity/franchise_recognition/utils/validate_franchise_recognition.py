#!/usr/bin/env python3
"""Валидатор franchise_recognition NPZ: схема, --struct, --qa (view_csv_feature_qa.json)."""
...
_K = 5
_SCHEMA = "franchise_recognition_npz_v2"
...
def validate_structure(npz_path: str) -> List[str]:
    ...
    if fids.shape != (n, _K) or fsc.shape != (n, _K):
        out.append(f"frame_topk_*: ож. ({n}, {_K}), ids={fids.shape}, sc={fsc.shape}")
    ...
    for i in range(n):
        for j in range(_K):
            ...
            if lid < 0:
                continue
            if lids and lid not in lids:
                out.append(f"frame_topk_ids[{i},{j}]={lid} вне label space")
            if np.isfinite(sc) and (sc < -0.0001 or sc > 1.0001):
                out.append(f"frame_topk_scores[{i},{j}]={sc} вне [0,1] (ES similarity)")

    tids = np.asarray(d["track_topk_ids"], dtype=np.int32)
    tsc = np.asarray(d["track_topk_scores"], dtype=np.float32)
    tev = np.asarray(d["track_topk_evidence_frame_indices"], dtype=np.int32)
    if tids.shape != (1, _K) or tsc.shape != (1, _K) or tev.shape != (1, _K):
        out.append(f"track_*: неверные формы: ids={tids.shape}, ev={tev.shape}")
    fi_set = set(int(x) for x in fi.tolist())
    for j in range(_K):
        ev = int(tev[0, j])
        if ev >= 0 and ev not in fi_set:
            out.append(f"track_topk_evidence_frame_indices[0,{j}]={ev} не из frame_indices")

    trk = np.asarray(d["track_ids"], dtype=np.int32).ravel()
    if trk.size != 1 or int(trk[0]) != 0:
        out.append(f"track_ids: ож. [0], got {trk.tolist()}")
...
def main() -> int:
    p = argparse.ArgumentParser(description="validate franchise_recognition NPZ (VisualProcessor)")
    p.add_argument("npz_path")
    p.add_argument("--qa", action="store_true", help="Плоский meta vs view_csv_feature_qa")
    p.add_argument("--struct", action="store_true", help="Ключи, формы, [0,1] scores, evidence")
    args = p.parse_args()
