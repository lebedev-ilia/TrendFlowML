...
class EmbeddingServiceClient:
    ...
    def _ensure_url(self):
        ...
        try:
            ...
            if isinstance(e, requests.exceptions.HTTPError) and e.response is None:
                raw = str(e)
                if "BadStatusLine" in raw or "Connection aborted" in raw:
                    tip += " — often: wrong port or non-HTTP server"
    ...
    def get_labels(
        ...
        """
        Get all labels for a category from Embedding Service.
        ...
        Args:
            category: Category name (e.g., "franchise", "brand", "car")
        ...
        try:
            ...
            raise RuntimeError(f"Embedding Service get_labels failed: {e}")
