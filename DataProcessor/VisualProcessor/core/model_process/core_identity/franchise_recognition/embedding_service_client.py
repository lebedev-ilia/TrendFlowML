...
class EmbeddingServiceClient:
    ...
    def _ensure_url(self):
        ...
        Raises:
            RuntimeError: If service is unavailable or health check fails
        """
        try:
            ...
            error_msg = str(e)
            if "BadStatusLine" in error_msg or "Connection aborted" in error_msg:
                raise RuntimeError(
                    f"Embedding Service connection failed at {self.base_url}. "
                    f"This usually means:\n"
                    f"  1. The service is not running\n"
                    f"  2. The service is using a different protocol (gRPC instead of HTTP)\n"
                    f"  3. The port is incorrect (expected HTTP on port 8001, but got binary protocol)\n"
                    f"Please check:\n"
                    f"  - Is Embedding Service running? (python embedding_service/run_server.py)\n"
                    f"  - Is it listening on HTTP port 8001? (check http://localhost:8001/health)\n"
                    f"  - Original error: {error_msg}"
                )
            raise RuntimeError(
                f"Embedding Service unavailable at {self.base_url}: {error_msg}. "
                f"Please ensure the service is running and accessible."
            )
        except Exception as e:
            raise RuntimeError(
                f"Embedding Service unavailable at {self.base_url}: {e}. "
                f"Please ensure the service is running and accessible at {self.base_url}/health"
            )
