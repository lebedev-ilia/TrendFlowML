#!/usr/bin/env python3
"""
Audit v4 / 4.2: статистика по `title_to_hashtag_cosine_extractor`.

Проверяет:
  - табличный срез `tp_titlehashcos_*` (11 ключей) в `text_processor/text_features.npz`;
  - наличие upstream артефактов `text_processor/_artifacts/title_embedding.npy` и `hashtag_embedding.npy`;
  - при `present=1`: согласование `tp_titlehashcos_cosine` с cosine, пересчитанным из `.npy` (L2-нормировка в скрипте).
...
PREFIX = "tp_titlehashcos_"
EXPECTED_KEYS = 11
TITLE_VEC = "title_embedding.npy"
HASHTAG_VEC = "hashtag_embedding.npy"
...
def _summarize_vec(path: Path) -> Dict[str, Any]:
    out: Dict[str, Any] = {"path": str(path), "exists": bool(path.is_file()), "shape": None, "dtype": None, "finite": None, "l2": None, "error": None}
    if not path.is_file():
        return out
    try:
        v = np.load(path)
...
def _cosine_from_files(title_path: Path, hashtag_path: Path) -> Optional[float]:
    try:
        a = np.asarray(np.load(title_path), dtype=np.float32).reshape(-1)
        b = np.asarray(np.load(hashtag_path), dtype=np.float32).reshape(-1)
        if a.size <= 0 or b.size <= 0 or int(a.size) != int(b.size):
            return None
...
def summarize_text_npz(path: Path) -> Dict[str, Any]:
    ...
    title_path = artifacts_dir / TITLE_VEC
    hashtag_path = artifacts_dir / HASHTAG_VEC
    ...
    out: Dict[str, Any] = {
        ...
        "upstream_title_embedding": _summarize_vec(title_path),
        "upstream_hashtag_embedding": _summarize_vec(hashtag_path),
        "consistency": {},
    }
    ...
    try:
        ...
        cosine_tab = pair.get("tp_titlehashcos_cosine")
        present = pair.get("tp_titlehashcos_present")
        present_i = None
        if isinstance(present, (int, float, np.integer, np.floating)) and np.isfinite(float(present)):
            if float(int(float(present))) == float(present):
                present_i = int(float(present))

        cosine_calc = _cosine_from_files(title_path, hashtag_path) if present_i == 1 else None
        cosine_diff = None
        if cosine_calc is not None and isinstance(cosine_tab, (int, float, np.integer, np.floating)) and np.isfinite(float(cosine_tab)):
            cosine_diff = float(abs(float(cosine_tab) - float(cosine_calc)))
        ...
        out["derived"] = {
            ...
            "tp_titlehashcos_present": pair.get("tp_titlehashcos_present"),
            "tp_titlehashcos_title_present": pair.get("tp_titlehashcos_title_present"),
            "tp_titlehashcos_hashtag_present": pair.get("tp_titlehashcos_hashtag_present"),
        }
        out["consistency"] = {
            "cosine_tabular": float(cosine_tab) if isinstance(cosine_tab, (int, float, np.integer, np.floating)) and np.isfinite(float(cosine_tab)) else None,
            "cosine_recomputed": float(cosine_calc) if cosine_calc is not None else None,
            "abs_diff": float(cosine_diff) if cosine_diff is not None else None,
            "ok_if_present": bool(present_i != 1 or (cosine_diff is not None and cosine_diff <= 1e-4)),
        }
    finally:
        z.close()
    return out
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 stats: title_to_hashtag_cosine_extractor (tp_titlehashcos_* + upstream vectors)")
    ...
    row_ok = np.array(
        [
            ...
            and bool((s.get("upstream_hashtag_embedding") or {}).get("exists"))
            and bool((s.get("consistency") or {}).get("ok_if_present"))
            for s in per_file
        ],
        dtype=bool,
    )
    ...
    dataset_quality = {
        ...
        "n_text_processor_ok_with_slice_and_upstream_vectors": n_ok,
    ...
    report = {
        ...
        "prefix": PREFIX,
        "expected_keys": EXPECTED_KEYS,
        "upstream_vector_artifacts": [f"text_processor/_artifacts/{TITLE_VEC}", f"text_processor/_artifacts/{HASHTAG_VEC}"],
    ...
    json_path = args.out_dir / "title_to_hashtag_cosine_extractor_audit_v4_stats.json"
    json_path.write_text(json.dumps(_sanitize_json(report), ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {json_path}", file=sys.stderr)
    if n_ok < 3:
        print("Skipped plots: fewer than 3 OK text_processor runs with tp_titlehashcos slice", file=sys.stderr)
    return 0
