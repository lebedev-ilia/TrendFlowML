#!/usr/bin/env python3
"""
Audit v4 / 4.2: статистика по срезу `tp_embpair_*` + legacy `tp_pairtopk_*`
в `text_processor/text_features.npz`.

Компонент не пишет собственных `.npy` артефактов: вычисляет top-k по уже готовым
эмбеддингам (title/description/transcript chunk matrix) из `tp_artifacts`.

Пример:
  cd DataProcessor/TextProcessor
  ../.data_venv/bin/python \
    src/extractors/embedding_pair_topk_extractor/scripts/audit_v4_npz_stats.py \
    --out-dir ../../storage/audit_v4/embedding_pair_topk_extractor_l2 \
    --seed 0
"""
...
PREFIXES = ("tp_embpair_", "tp_pairtopk_")
EXPECTED_KEYS_TOTAL = 69
EXPECTED_CANON = 56
EXPECTED_LEGACY = 13
...
def summarize_text_npz(path: Path) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {
        ...
        "expected_keys_total": EXPECTED_KEYS_TOTAL,
        "expected_keys_canon": EXPECTED_CANON,
        "expected_keys_legacy": EXPECTED_LEGACY,
        "tabular_slice": {},
        "meta_flat": {},
    ...
    try:
        ...
        out["tabular_slice"] = {
            ...
            "n_keys_total": len(fn),
            "n_keys_canon": int(sum(1 for n in fn if n.startswith("tp_embpair_"))),
            "n_keys_legacy": int(sum(1 for n in fn if n.startswith("tp_pairtopk_"))),
        }
        ...
        out["derived"] = {
            ...
            "slice_ok_total": bool(out["tabular_slice"]["n_keys_total"] == EXPECTED_KEYS_TOTAL),
            "slice_ok_split": bool(
                out["tabular_slice"]["n_keys_canon"] == EXPECTED_CANON
                and out["tabular_slice"]["n_keys_legacy"] == EXPECTED_LEGACY
            ),
            "tp_embpair_present": float(out["tabular_slice"]["pairwise"].get("tp_embpair_present"))
            if "tp_embpair_present" in out["tabular_slice"]["pairwise"]
            else None,
        }
    finally:
        z.close()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 stats: tp_embpair_* + tp_pairtopk_* slice in text_features.npz")
    ...
    row_ok = np.array(
        [
            ...
            and s.get("derived", {}).get("slice_ok_total")
            and s.get("derived", {}).get("slice_ok_split")
            for s in per_file
        ],
        dtype=bool,
    )
    ...
    json_path = args.out_dir / "embedding_pair_topk_extractor_audit_v4_stats.json"
    json_path.write_text(json.dumps(_sanitize_json(report), ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {json_path}", file=sys.stderr)
    if n_ok < 3:
        print("Skipped plots: fewer than 3 OK text_processor runs with embpair/pairtopk slice", file=sys.stderr)
    return 0
