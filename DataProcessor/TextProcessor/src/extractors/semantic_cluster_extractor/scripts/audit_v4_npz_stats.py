#!/usr/bin/env python3
"""
Audit v4 / 4.2: статистика по срезу `tp_semclust_*` в `text_processor/text_features.npz`.

Компонент не пишет собственных `.npy` артефактов: использует upstream эмбеддинги из tp_artifacts
и dp_models assets (PCA + centroids). Для L2 фиксируем табличный контракт (31 ключ) и качество набора.

Пример:
  cd DataProcessor/TextProcessor
  ../.data_venv/bin/python \
    src/extractors/semantic_cluster_extractor/scripts/audit_v4_npz_stats.py \
    --out-dir ../../storage/audit_v4/semantic_cluster_extractor_l2 \
    --seed 0
"""
...
def summarize_text_npz(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        out["derived"] = {
            ...
            "tp_semclust_present": float(out["tabular_slice"]["pairwise"].get("tp_semclust_present"))
            if "tp_semclust_present" in out["tabular_slice"]["pairwise"]
            else None,
        }
    finally:
        z.close()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 stats: tp_semclust_* slice in text_features.npz")
    ...
    json_path = args.out_dir / "semantic_cluster_extractor_audit_v4_stats.json"
    json_path.write_text(json.dumps(_sanitize_json(report), ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {json_path}", file=sys.stderr)
    if n_ok < 3:
        print("Skipped plots: fewer than 3 OK text_processor runs with tp_semclust slice", file=sys.stderr)
    return 0
