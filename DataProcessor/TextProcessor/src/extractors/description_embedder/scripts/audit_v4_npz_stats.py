#!/usr/bin/env python3
"""
Audit v4 / 4.2: статистика по срезу `tp_descemb_*` в `text_processor/text_features.npz`.

TextProcessor пишет табличный слой в общий `text_features.npz`, а вектор эмбеддинга — в
`text_processor/_artifacts/description_embedding.npy`.

Скрипт:
  - читает feature_names/feature_values;
  - выделяет только `tp_descemb_*` (контракт description_embedder_output_v1);
  - фиксирует качество набора (meta.status ok/error);
  - проверяет артефакт `.npy` по форме/конечности/L2.

Пример:
  cd DataProcessor/TextProcessor
  ../.data_venv/bin/python \
    src/extractors/description_embedder/scripts/audit_v4_npz_stats.py \
    --out-dir ../../storage/audit_v4/description_embedder_l2 \
    --seed 0
"""
...
def _artifact_path_from_npz(npz_path: Path) -> Path:
    return npz_path.parent / "_artifacts" / "description_embedding.npy"


def _summarize_embedding(path: Path) -> Dict[str, Any]:
    out: Dict[str, Any] = {"exists": bool(path.is_file()), "shape": None, "dtype": None, "finite": None, "l2": None}
    if not path.is_file():
        return out
    arr = np.load(path)
    a = np.asarray(arr)
    ...
    if a.size == 0:
        ...
        out["l2"] = 0.0
        return out
    out["finite"] = bool(np.isfinite(a).all())
    v = a.astype(np.float64).reshape(-1)
    out["l2"] = float(np.linalg.norm(v))
    return out
...
def summarize_text_npz(path: Path) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {
        ...
        "artifact_description_embedding": None,
    }
    ...
    try:
        ...
        out["derived"] = {
            ...
            "tp_descemb_present": float(out["tabular_slice"]["pairwise"].get("tp_descemb_present"))
            if "tp_descemb_present" in out["tabular_slice"]["pairwise"]
            else None,
        }

        art = _artifact_path_from_npz(path)
        out["artifact_description_embedding"] = {"path": str(art), **_summarize_embedding(art)}
    finally:
        z.close()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 stats: tp_descemb_* slice in text_features.npz")
    ...
    json_path = args.out_dir / "description_embedder_audit_v4_stats.json"
    json_path.write_text(json.dumps(_sanitize_json(report), ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {json_path}", file=sys.stderr)
    if n_ok < 3:
        print("Skipped plots: fewer than 3 OK text_processor runs with tp_descemb slice", file=sys.stderr)
    return 0
