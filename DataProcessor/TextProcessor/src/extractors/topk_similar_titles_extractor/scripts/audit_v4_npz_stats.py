#!/usr/bin/env python3
"""
Audit v4 / 4.2: статистика по `topk_similar_titles_extractor`.

Проверяет:
  - табличный срез `tp_topktitles_*` (29 ключей) в `text_processor/text_features.npz`;
  - наличие upstream `text_processor/_artifacts/title_embedding.npy` (если text_processor ok).
...
def summarize_text_npz(path: Path) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {
        ...
        "upstream_title_embedding": _summarize_vec(vec_path),
    }
    ...
    try:
        ...
        out["derived"] = {
            ...
            "tp_topktitles_present": pair.get("tp_topktitles_present"),
            "tp_topktitles_enabled": pair.get("tp_topktitles_enabled"),
            "tp_topktitles_backend_faiss": pair.get("tp_topktitles_backend_faiss"),
        }
    finally:
        z.close()
    return out
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 stats: topk_similar_titles_extractor (tp_topktitles_*)")
    ...
    dataset_quality = {
        ...
        "n_text_processor_ok_with_slice_and_upstream_vector": n_ok,
    ...
    json_path = args.out_dir / "topk_similar_titles_extractor_audit_v4_stats.json"
    json_path.write_text(json.dumps(_sanitize_json(report), ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {json_path}", file=sys.stderr)
    if n_ok < 3:
        print("Skipped plots: fewer than 3 OK text_processor runs with tp_topktitles slice", file=sys.stderr)
    return 0
