#!/usr/bin/env python3
"""
Audit v4 / 4.2: статистика по срезу `tp_embshift_*` в `text_processor/text_features.npz`.

Компонент читает матрицу чанк-эмбеддингов транскрипта из upstream `.npy` (через tp_artifacts),
но в Audit v4 фиксируем именно табличный контракт (27 ключей) и качество `result_store`.

Пример:
  cd DataProcessor/TextProcessor
  ../.data_venv/bin/python \
    src/extractors/embedding_shift_indicator_extractor/scripts/audit_v4_npz_stats.py \
    --out-dir ../../storage/audit_v4/embedding_shift_indicator_extractor_l2 \
    --seed 0
"""
...
def summarize_text_npz(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        out["derived"] = {
            ...
            "tp_embshift_present": float(out["tabular_slice"]["pairwise"].get("tp_embshift_present"))
            if "tp_embshift_present" in out["tabular_slice"]["pairwise"]
            else None,
        }
    finally:
        z.close()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 stats: tp_embshift_* slice in text_features.npz")
    ...
    json_path = args.out_dir / "embedding_shift_indicator_extractor_audit_v4_stats.json"
    json_path.write_text(json.dumps(_sanitize_json(report), ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {json_path}", file=sys.stderr)
    if n_ok < 3:
        print("Skipped plots: fewer than 3 OK text_processor runs with tp_embshift slice", file=sys.stderr)
    return 0
