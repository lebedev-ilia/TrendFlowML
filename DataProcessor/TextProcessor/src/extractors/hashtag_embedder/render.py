"""
Renderer для hashtag_embedder extractor: генерация render-context JSON и HTML debug страницы.

Генерирует human-friendly представление эмбеддингов хештегов для визуализации и LLM.
"""
...
def render_hashtag_embedder(
    extractor_features: Dict[str, Any],
    payload: Dict[str, Any],
    meta: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Генерировать render-context для hashtag_embedder extractor.
    
    Args:
        extractor_features: Словарь фич этого extractor'а (префикс "tp_hashemb_")
    ...
    for key, value in extractor_features.items():
        if key in ["tp_hashemb_present", "tp_hashemb_dim", "tp_hashemb_l2_norm"]:
            feature_name = key.replace("tp_hashemb_", "")
            embedding[feature_name] = _clean_value(value)
    
    # Extract tags features
    tags = {}
    for key, value in extractor_features.items():
        if key.startswith("tp_hashemb_tag_count") or key.startswith("tp_hashemb_n_input_tags") or key.startswith("tp_hashemb_n_unique_tags") or key.startswith("tp_hashemb_n_tags_truncated"):
            feature_name = key.replace("tp_hashemb_", "")
            tags[feature_name] = _clean_value(value)
    
    # Extract aggregation features
    aggregation = {}
    for key, value in extractor_features.items():
        if key.startswith("tp_hashemb_agg_") or key.startswith("tp_hashemb_use_frequencies"):
            feature_name = key.replace("tp_hashemb_", "")
    ...
    cache = {}
    for key, value in extractor_features.items():
        if key.startswith("tp_hashemb_cache") or key.startswith("tp_hashemb_artifact"):
            feature_name = key.replace("tp_hashemb_", "")
    ...
    performance = {}
    for key, value in extractor_features.items():
        if key.startswith("tp_hashemb_encode_ms") or key.startswith("tp_hashemb_agg_ms") or key.startswith("tp_hashemb_device") or key.startswith("tp_hashemb_fp16"):
            feature_name = key.replace("tp_hashemb_", "")
    ...
    configuration = {}
    for key, value in extractor_features.items():
        if key.startswith("tp_hashemb_") and not any(key.startswith(prefix) for prefix in [
            "tp_hashemb_present", "tp_hashemb_dim", "tp_hashemb_l2_norm",
            "tp_hashemb_tag_count", "tp_hashemb_n_input_tags", "tp_hashemb_n_unique_tags", "tp_hashemb_n_tags_truncated",
            "tp_hashemb_cache", "tp_hashemb_artifact", "tp_hashemb_encode_ms", "tp_hashemb_agg_ms",
            "tp_hashemb_device", "tp_hashemb_fp16", "tp_hashemb_agg_", "tp_hashemb_use_frequencies"
        ]):
            feature_name = key.replace("tp_hashemb_", "")
    ...
    agg_strategy = "mean"
    if aggregation.get("agg_max", 0) > 0.5:
        agg_strategy = "max"
    elif aggregation.get("agg_logsumexp", 0) > 0.5:
        agg_strategy = "logsumexp"
    ...
    render["summary"] = {
        ...
        "tag_count": _clean_value(tags.get("tag_count")),
        "n_input_tags": _clean_value(tags.get("n_input_tags")),
        "n_unique_tags": _clean_value(tags.get("n_unique_tags")),
        "n_tags_truncated": _clean_value(tags.get("n_tags_truncated")),
        "aggregation_strategy": agg_strategy,
        "use_frequencies": bool(aggregation.get("use_frequencies", 0) > 0.5),
        ...
        "agg_time_ms": _clean_value(performance.get("agg_ms")),
...
def render_hashtag_embedder_html(
    npz_path: str,
    output_path: str,
    extractor_features: Dict[str, Any],
    payload: Dict[str, Any],
    ...
    """
    Генерировать HTML страницу для дебага hashtag_embedder результатов.
    ...
    """
    render = render_hashtag_embedder(extractor_features, payload, meta)
    
    summary = render.get("summary", {})
    embedding = render.get("embedding", {})
    tags = render.get("tags", {})
    ...
    # Tags statistics
    tags_stats = {}
    if tags.get("n_input_tags") is not None:
        tags_stats["Входных тегов"] = tags.get("n_input_tags")
    if tags.get("n_unique_tags") is not None:
        tags_stats["Уникальных тегов"] = tags.get("n_unique_tags")
    if tags.get("tag_count") is not None:
        tags_stats["Использовано тегов"] = tags.get("tag_count")
    if tags.get("n_tags_truncated") is not None and tags.get("n_tags_truncated", 0) > 0:
        tags_stats["Обрезано тегов"] = tags.get("n_tags_truncated")
    
    # Performance metrics
    perf_metrics = {}
    if performance.get("encode_ms") is not None:
        perf_metrics["Кодирование (мс)"] = performance.get("encode_ms")
    if performance.get("agg_ms") is not None:
        perf_metrics["Агрегация (мс)"] = performance.get("agg_ms")
    
    html_content = f"""<!DOCTYPE html>
<html>
<head>
    <title>Эмбеддинги хештегов</title>
    <meta charset="UTF-8">
...
<body>
    <div class="container">
        <h1>Эмбеддинги хештегов</h1>
        <p><strong>Компонент:</strong> {render.get("component", "hashtag_embedder")}</p>
        ...
        <p><em>L2-нормализованный эмбеддинг для хештегов видео (агрегация по списку хештегов) с использованием sentence transformers</em></p>
        ...
        <div class="section">
            ...
            <div class="feature-grid">
                ...
                <div class="feature-card">
                    <div class="feature-name">Количество тегов</div>
                    <div class="feature-value">{summary.get("tag_count", "N/A")}</div>
                </div>
                <div class="feature-card">
                    <div class="feature-name">Входных тегов</div>
                    <div class="feature-value">{summary.get("n_input_tags", "N/A")}</div>
                </div>
                <div class="feature-card">
                    <div class="feature-name">Уникальных тегов</div>
                    <div class="feature-value">{summary.get("n_unique_tags", "N/A")}</div>
                </div>
                <div class="feature-card">
                    <div class="feature-name">Обрезано тегов</div>
                    <div class="feature-value">{summary.get("n_tags_truncated", "N/A")}</div>
                </div>
                <div class="feature-card">
                    <div class="feature-name">Стратегия агрегации</div>
                    <div class="feature-value">{summary.get("aggregation_strategy", "N/A")}</div>
                </div>
                <div class="feature-card">
                    <div class="feature-name">Использование частот</div>
                    <div class="feature-value">{'Да' if summary.get("use_frequencies") else 'Нет'}</div>
                </div>
                <div class="feature-card">
                ...
                <div class="feature-card">
                    <div class="feature-name">Время агрегации (мс)</div>
                    <div class="feature-value">{summary.get("agg_time_ms", "N/A")}</div>
                </div>
                <div class="feature-card">
        ...
        <div class="section">
            ...
            <table>
                ...
                <tr>
                    <td>Входных тегов</td>
                    <td>{tags.get("n_input_tags", "N/A")}</td>
                    <td>Количество хештегов во входных данных (до обработки)</td>
                </tr>
                <tr>
                    <td>Уникальных тегов</td>
                    <td>{tags.get("n_unique_tags", "N/A")}</td>
                    <td>Количество уникальных хештегов после canonicalization (dedup, sort)</td>
                </tr>
                <tr>
                    <td>Использовано тегов</td>
                    <td>{tags.get("tag_count", "N/A")}</td>
                    <td>Количество хештегов, использованных для создания эмбеддинга (после truncate до max_tags)</td>
                </tr>
                <tr>
                    <td>Обрезано тегов</td>
                    <td>{tags.get("n_tags_truncated", "N/A")}</td>
                    <td>Количество хештегов, которые были обрезаны из-за лимита max_tags</td>
                </tr>
            </table>
        </div>
        ...
        <div class="section">
            ...
            <table>
                ...
                <tr>
                    <td>Стратегия агрегации</td>
                    <td>{summary.get("aggregation_strategy", "N/A")}</td>
                    <td>Метод агрегации эмбеддингов отдельных хештегов: mean (среднее), max (максимум), logsumexp (log-sum-exp для стабильности)</td>
                </tr>
                <tr>
                    <td>Использование частот</td>
                    <td>{'Да' if aggregation.get("use_frequencies", 0) > 0.5 else 'Нет'}</td>
                    <td>Учитываются ли дубликаты хештегов как веса при агрегации (если True, то частые хештеги имеют больший вес)</td>
                </tr>
            </table>
        </div>
        ...
        <div class="section">
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>Время, затраченное на кодирование всех хештегов через модель (в миллисекундах)</td>
                </tr>
                <tr>
                    <td>Время агрегации (мс)</td>
                    <td>{performance.get("agg_ms", "N/A")}</td>
                    <td>Время, затраченное на агрегацию эмбеддингов хештегов в финальный вектор</td>
                </tr>
                <tr>
                    <td>Устройство</td>
        ...
        <div class="section">
            ...
            <div class="feature-grid">
                <div class="feature-card">
                    <div class="feature-name">Хештеги обязательны</div>
                    <div class="feature-value">{'Да' if configuration.get("require_hashtags_enabled", 0) > 0.5 else 'Нет'}</div>
                </div>
                <div class="feature-card">
    ...
    <script>
        ...
        // График статистики хештегов
        var tagsStats = {json.dumps(tags_stats)};
        if (Object.keys(tagsStats).length > 0) {{
            var trace2 = {{
                x: Object.keys(tagsStats),
                y: Object.values(tagsStats),
                type: 'bar',
                marker: {{ color: '#2196F3' }},
                name: 'Количество'
            }};
            var layout2 = {{
                title: 'Статистика хештегов',
                xaxis: {{ title: 'Тип' }},
                yaxis: {{ title: 'Количество' }}
            }};
            Plotly.newPlot('tags-plot', [trace2], layout2);
        }}
        ...
        if (Object.keys(perfMetrics).length > 0) {{
            ...
            Plotly.newPlot('performance-plot', [trace3], layout3);
        }}
    </script>
</body>
</html>
"""
    ...
    logger.info(f"Hashtag embedder HTML render saved to {output_path}")
    return output_path


__all__ = ["render_hashtag_embedder", "render_hashtag_embedder_html"]
