#!/usr/bin/env python3
"""
Audit v4 / 4.2: статистика по `hashtag_embedder`.

Проверяет:
  - табличный срез `tp_hashemb_*` (23 ключа) в `text_processor/text_features.npz`;
  - артефакт `text_processor/_artifacts/hashtag_embedding.npy` (shape/dtype/L2);
  - согласованность `tp_hashemb_dim` с размерностью `.npy` (если есть).

Пример:
  cd DataProcessor/TextProcessor
  ../.data_venv/bin/python \
    src/extractors/hashtag_embedder/scripts/audit_v4_npz_stats.py \
    --out-dir ../../storage/audit_v4/hashtag_embedder_l2 \
    --seed 0
"""
...
PREFIX = "tp_hashemb_"
EXPECTED_KEYS = 23
ARTIFACT_NAME = "hashtag_embedding.npy"
...
def _summarize_vector(path: Path) -> Dict[str, Any]:
    out: Dict[str, Any] = {
        "path": str(path),
...
def summarize_text_npz(path: Path) -> Dict[str, Any]:
    ...
    vec_path = artifacts_dir / ARTIFACT_NAME
    ...
    out: Dict[str, Any] = {
        ...
        "artifact_vector": _summarize_vector(vec_path),
        "consistency": {},
    }
    ...
    try:
        ...
        out["derived"] = {
            ...
            "tp_hashemb_present": float(out["tabular_slice"]["pairwise"].get("tp_hashemb_present"))
            if "tp_hashemb_present" in out["tabular_slice"]["pairwise"]
            else None,
        }

        dim_tab = out["tabular_slice"]["pairwise"].get("tp_hashemb_dim")
        dim_tab_i = int(dim_tab) if isinstance(dim_tab, (int, float)) and float(dim_tab) == float(int(dim_tab)) else None
        vec_shape = out["artifact_vector"].get("shape")
        vec_dim = int(vec_shape[0]) if isinstance(vec_shape, list) and len(vec_shape) == 1 else None
        out["consistency"] = {
            "tp_hashemb_dim": dim_tab_i,
            "artifact_dim": vec_dim,
            "dim_match": bool(dim_tab_i is not None and vec_dim is not None and dim_tab_i == vec_dim),
            "artifact_expected_when_present": bool(
                out["derived"]["tp_hashemb_present"] == 1.0 if out.get("derived") else False
            ),
        }
    finally:
        z.close()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 stats: hashtag_embedder (tp_hashemb_* + hashtag_embedding.npy)")
    ...
    row_ok = np.array(
        [
            ...
            and bool((s.get("artifact_vector") or {}).get("exists"))
            and bool((s.get("artifact_vector") or {}).get("finite"))
            and bool((s.get("consistency") or {}).get("dim_match"))
            for s in per_file
        ],
        dtype=bool,
    )
    ...
    report = {
        ...
        "prefix": PREFIX,
        "expected_keys": EXPECTED_KEYS,
        "vector_artifact": f"text_processor/_artifacts/{ARTIFACT_NAME}",
    ...
    json_path = args.out_dir / "hashtag_embedder_audit_v4_stats.json"
    json_path.write_text(json.dumps(_sanitize_json(report), ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {json_path}", file=sys.stderr)
    if n_ok < 3:
        print("Skipped plots: fewer than 3 OK text_processor runs with tp_hashemb slice", file=sys.stderr)
    return 0
