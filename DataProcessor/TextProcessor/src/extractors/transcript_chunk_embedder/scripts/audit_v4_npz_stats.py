#!/usr/bin/env python3
"""
Audit v4 / 4.2: статистика по `transcript_chunk_embedder`.

Проверяет:
  - табличный срез `tp_tchunk_*` (16 ключей) в `text_processor/text_features.npz`;
  - артефакт `text_processor/_artifacts/transcript_whisper_chunk_embeddings.npy` (shape/dtype/finite + L2 нормы строк);
  - консистентность `tp_tchunk_embedding_dim` и второй размерности матрицы.
...
PREFIX = "tp_tchunk_"
EXPECTED_KEYS = 16
ARTIFACT = "transcript_whisper_chunk_embeddings.npy"
...
def _summarize_matrix(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        if m.ndim == 2 and m.shape[0] > 0:
            ...
            out["row_l2_min"] = float(np.min(norms))
            out["row_l2_mean"] = float(np.mean(norms))
            out["row_l2_max"] = float(np.max(norms))
        return out
...
def summarize_text_npz(path: Path) -> Dict[str, Any]:
    ...
    tp_dir = path.parent
    art_path = tp_dir / "_artifacts" / ARTIFACT
    ...
    out: Dict[str, Any] = {
        ...
        "artifact_matrix": _summarize_matrix(art_path),
        "consistency": {},
    }
    ...
    try:
        ...
        dim_tab = _to_int_if_finite(pair.get("tp_tchunk_embedding_dim"))
        mat_shape = out["artifact_matrix"].get("shape")
        mat_dim = int(mat_shape[1]) if isinstance(mat_shape, list) and len(mat_shape) == 2 else None
        ...
        out["derived"] = {
            ...
            "tp_tchunk_present": pair.get("tp_tchunk_present"),
            "tp_tchunk_whisper_chunks": pair.get("tp_tchunk_whisper_chunks"),
        }
        out["consistency"] = {
            "tp_tchunk_embedding_dim": dim_tab,
            "artifact_dim": mat_dim,
            "dim_match": bool(dim_tab is not None and mat_dim is not None and dim_tab == mat_dim),
        }
    finally:
        z.close()
    return out
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 stats: transcript_chunk_embedder (tp_tchunk_* + matrix artifact)")
    ...
    row_ok = np.array(
        [
            ...
            and bool((s.get("artifact_matrix") or {}).get("exists"))
            and bool((s.get("artifact_matrix") or {}).get("finite"))
    ...
    report = {
        ...
        "prefix": PREFIX,
        "expected_keys": EXPECTED_KEYS,
        "matrix_artifact": f"text_processor/_artifacts/{ARTIFACT}",
    ...
    json_path = args.out_dir / "transcript_chunk_embedder_audit_v4_stats.json"
    json_path.write_text(json.dumps(_sanitize_json(report), ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {json_path}", file=sys.stderr)
    if n_ok < 3:
        print("Skipped plots: fewer than 3 OK text_processor runs with tp_tchunk slice", file=sys.stderr)
    return 0
