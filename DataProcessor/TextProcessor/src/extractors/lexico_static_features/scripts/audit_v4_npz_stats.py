#!/usr/bin/env python3
"""
Audit v4 / 4.2: статистика по срезу `tp_lex_*` в `text_processor/text_features.npz`.

Компонент не пишет отдельных `.npy` артефактов: весь контракт — табличные скаляры.
В L2 фиксируем:
  - наличие полного среза (ожидаемо 67 ключей);
  - долю NaN/finite по фичам;
  - качество набора (meta.status ok/error) на 5 путях A+B.

Пример:
  cd DataProcessor/TextProcessor
  ../.data_venv/bin/python \
    src/extractors/lexico_static_features/scripts/audit_v4_npz_stats.py \
    --out-dir ../../storage/audit_v4/lexico_static_features_l2 \
    --seed 0
"""
...
def summarize_text_npz(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        out["derived"] = {
            ...
            "tp_lex_enabled": float(out["tabular_slice"]["pairwise"].get("tp_lex_enabled"))
            if "tp_lex_enabled" in out["tabular_slice"]["pairwise"]
            else None,
        }
    finally:
        z.close()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 stats: tp_lex_* slice in text_features.npz")
    ...
    json_path = args.out_dir / "lexico_static_features_audit_v4_stats.json"
    json_path.write_text(json.dumps(_sanitize_json(report), ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {json_path}", file=sys.stderr)
    if n_ok < 3:
        print("Skipped plots: fewer than 3 OK text_processor runs with tp_lex slice", file=sys.stderr)
    return 0
