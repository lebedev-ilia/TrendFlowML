#!/usr/bin/env python3
"""
Audit v4 / 4.2: статистика по `comments_aggregator` в `text_processor/text_features.npz`.

Выделяет 39 ключей из трёх префиксов:
  - tp_commentsagg_* (canonical)
  - tp_comments_agg_* (legacy)
  - tp_cagg_* (legacy)

Также проверяет бинарные артефакты per-run:
  - comments_embeddings.npy (N, D)
  - comments_agg_mean.npy (D,)
  - comments_agg_median.npy (D,)
  - comments_selected_indices.npy (N,)

Пример:
  cd DataProcessor/TextProcessor
  ../.data_venv/bin/python \
    src/extractors/comments_aggregator/scripts/audit_v4_npz_stats.py \
    --out-dir ../../storage/audit_v4/comments_aggregator_l2 \
    --seed 0
"""
...
PREFIXES: Tuple[str, ...] = ("tp_commentsagg_", "tp_comments_agg_", "tp_cagg_")
EXPECTED_KEYS = 39
...
def _is_key_of_interest(name: str) -> bool:
    return any(name.startswith(p) for p in PREFIXES)


def _filter_tabular(names: List[str], vals: np.ndarray) -> Tuple[List[str], np.ndarray]:
    idx = [i for i, n in enumerate(names) if _is_key_of_interest(n)]
...
def _artifacts_dir(npz_path: Path) -> Path:
    return npz_path.parent / "_artifacts"


def _summarize_npy(path: Path) -> Dict[str, Any]:
    out: Dict[str, Any] = {"exists": bool(path.is_file()), "shape": None, "dtype": None, "finite": None}
    if not path.is_file():
        return out
    arr = np.load(path)
    a = np.asarray(arr)
    ...
    out["finite"] = bool(np.isfinite(a).all()) if a.size else True
    return out
...
def summarize_text_npz(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        out["derived"] = {
            ...
            "tp_commentsagg_present": float(out["tabular_slice"]["pairwise"].get("tp_commentsagg_present"))
            if "tp_commentsagg_present" in out["tabular_slice"]["pairwise"]
            else None,
        }

        ad = _artifacts_dir(path)
        out["artifacts"] = {
            "comments_embeddings.npy": {"path": str(ad / "comments_embeddings.npy"), **_summarize_npy(ad / "comments_embeddings.npy")},
            "comments_agg_mean.npy": {"path": str(ad / "comments_agg_mean.npy"), **_summarize_npy(ad / "comments_agg_mean.npy")},
            "comments_agg_median.npy": {"path": str(ad / "comments_agg_median.npy"), **_summarize_npy(ad / "comments_agg_median.npy")},
            "comments_selected_indices.npy": {"path": str(ad / "comments_selected_indices.npy"), **_summarize_npy(ad / "comments_selected_indices.npy")},
        }
    finally:
        z.close()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 stats: comments_aggregator slice in text_features.npz")
    ...
    row_ok = np.array(
        [s.get("meta_flat", {}).get("status") == "ok" and s.get("derived", {}).get("slice_ok") for s in per_file],
        dtype=bool,
    )
    n_ok = int(row_ok.sum())
    ...
    json_path = args.out_dir / "comments_aggregator_audit_v4_stats.json"
    json_path.write_text(json.dumps(_sanitize_json(report), ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {json_path}", file=sys.stderr)
    if n_ok < 3:
        print("Skipped plots: fewer than 3 OK text_processor runs with comments_aggregator slice", file=sys.stderr)
    return 0
