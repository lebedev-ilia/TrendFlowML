#!/usr/bin/env python3
"""
Audit v4 / 4.2: статистика по `semantics_topics_keyphrases`.

Проверяет:
  - табличный срез `tp_topics_*` (116 ключей) в `text_processor/text_features.npz`;
  - артефакт `text_processor/_artifacts/tp_topics_keyphrase_embeddings.npy`:
      - если `tp_topics_present=1` и `tp_topics_keyphrases_count>0` → ожидается shape (K, D),
        где K==keyphrases_count, D==keyphrases_dim.

Важно: на некоторых error-run артефакт может быть записан частично (до падения пайплайна),
поэтому признак "OK" в L2 считаем только по `meta.status=ok` + полный tabular slice + валидность артефакта.

Пример:
  cd DataProcessor/TextProcessor
  ../.data_venv/bin/python \
    src/extractors/semantics_topics_keyphrases/scripts/audit_v4_npz_stats.py \
    --out-dir ../../storage/audit_v4/semantics_topics_keyphrases_l2 \
    --seed 0
"""
...
PREFIX = "tp_topics_"
EXPECTED_KEYS = 116
ARTIFACT_NAME = "tp_topics_keyphrase_embeddings.npy"
...
def summarize_text_npz(path: Path) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {
        ...
        "artifact_keyphrase_embeddings": _summarize_matrix(emb_path),
        "consistency": {},
    }
    ...
    try:
        ...
        present = pair.get("tp_topics_present")
        kp_count = pair.get("tp_topics_keyphrases_count")
        kp_dim = pair.get("tp_topics_keyphrases_dim")
        present_i = _to_int_if_finite(present)
        if present_i not in (0, 1):
            present_i = None
        kp_count_i = _to_int_if_finite(kp_count)
        kp_dim_i = _to_int_if_finite(kp_dim)
        ...
        out["derived"] = {
            ...
            "tp_topics_present": present,
            "tp_topics_keyphrases_count": kp_count,
            "tp_topics_keyphrases_dim": kp_dim,
        }

        emb = out["artifact_keyphrase_embeddings"]
        emb_exists = bool(emb.get("exists"))
        emb_shape = emb.get("shape")
        emb_k = int(emb_shape[0]) if isinstance(emb_shape, list) and len(emb_shape) == 2 else None
        emb_d = int(emb_shape[1]) if isinstance(emb_shape, list) and len(emb_shape) == 2 else None

        expects_emb = bool(present_i == 1 and (kp_count_i is not None) and kp_count_i > 0)
        ok_if_expected = expects_emb and emb_exists and (emb_k == kp_count_i) and (kp_dim_i is None or emb_d == kp_dim_i)
        ok_if_not_expected = (not expects_emb)
        out["consistency"] = {
            "present_i": present_i,
            "keyphrases_count_i": kp_count_i,
            "keyphrases_dim_i": kp_dim_i,
            "expects_embeddings_artifact": expects_emb,
            "artifact_exists": emb_exists,
            "artifact_k": emb_k,
            "artifact_d": emb_d,
            "artifact_matches_tabular": bool(ok_if_expected or ok_if_not_expected),
            "note": "artifact required when present=1 and keyphrases_count>0; otherwise may be absent/present due to partial runs.",
        }
    finally:
        z.close()
    return out
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 stats: semantics_topics_keyphrases (tp_topics_* + kp embeddings)")
    ...
    row_ok = np.array(
        [
            ...
            and bool((s.get("artifact_keyphrase_embeddings") or {}).get("exists"))
            and bool((s.get("artifact_keyphrase_embeddings") or {}).get("finite"))
    ...
    dataset_quality = {
        ...
        "n_text_processor_ok_with_slice_and_artifact": n_ok,
    ...
    report = {
        ...
        "prefix": PREFIX,
        "expected_keys": EXPECTED_KEYS,
        "keyphrase_embeddings_artifact": f"text_processor/_artifacts/{ARTIFACT_NAME}",
    ...
    json_path = args.out_dir / "semantics_topics_keyphrases_audit_v4_stats.json"
    json_path.write_text(json.dumps(_sanitize_json(report), ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {json_path}", file=sys.stderr)
    if n_ok < 3:
        print("Skipped plots: fewer than 3 OK text_processor runs with tp_topics slice", file=sys.stderr)
    return 0
