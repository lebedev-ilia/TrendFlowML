#!/usr/bin/env python3
"""
Статистика по срезу `tp_asrproxy_*` в `text_processor/text_features.npz` (Audit v4 L2).

Собирает несколько прогонов (A+B), строит per-file сводку, агрегат и корреляциитолько по полям префикса `tp_asrproxy_` (контракт asr_text_proxy_audio_features_output_v1).

Пример (набор B из RUN_LOG, reproducible A = первый путь):

  cd DataProcessor/TextProcessor
  ../.data_venv/bin/python \\
    src/extractors/asr_text_proxy_audio_features/scripts/audit_v4_npz_stats.py \\
    --out-dir ../../storage/audit_v4/asr_text_proxy_audio_features_l2 \\
    --seed 0
"""
...
# Набор L2 по умолчанию: тот же, что в RUN_LOG для Visual/Audio (mock e2e).
...
def summarize_text_npz(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        out["derived"] = {
            "nan_count_proxy_fields": int(nan_mask.sum()) if fv.size else 0,
            "finite_count_proxy_fields": int((~nan_mask).sum()) if fv.size else 0,
            "tp_asrproxy_present": float(fv[fn.index("tp_asrproxy_present")]) if fv.size and "tp_asrproxy_present" in fn else None,
            "proxy_keys_expected": 37,
            "proxy_slice_ok": bool(fv.size == 37),
        }
    finally:
        z.close()
    return out
...
def _stack_proxy(summaries: Sequence[Mapping[str, Any]]) -> Tuple[List[str], np.ndarray]:
    all_names: List[str] = []
    seen = set()
    for s in summaries:
        for n in s.get("tabular_proxy", {}).get("names", []):
            if n not in seen:
                seen.add(n)
                all_names.append(n)
    M = len(summaries)
    ...
    for r, s in enumerate(summaries):
        names = s.get("tabular_proxy", {}).get("names", [])
        vals = s.get("tabular_proxy", {}).get("values", [])
...
def _try_plots(names: List[str], mat: np.ndarray, out_dir: Path, *, max_features: int) -> None:
    import matplotlib
    ...
    for j, name in enumerate(names):
        ...
        plt.hist(col, bins=min(20, max(5, col.size)), color="#2a4365", edgecolor="white")
        plt.title(name[:80])
        plt.xlabel("value")
        plt.ylabel("count (runs)")
        plt.tight_layout()
        safe = "".join(c if c.isalnum() or c in "._-" else "_" for c in name)[:60]
        plt.savefig(fig_dir / f"hist_{j:03d}_{safe}.png", dpi=120)
        plt.close()
        k += 1


def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 L2 stats: tp_asrproxy_* in text_features.npz")
    p.add_argument(
        "--out-dir",
        type=Path,
        required=True,
        help="Каталог для JSON (и figures/)",
    )
    p.add_argument(
        "--npz",
        type=Path,
    ...
    p.add_argument("--max-plot-features", type=int, default=20)
    ...
    if args.npz:
        ...
        paths = [repo / rel for rel in _DEFAULT_NPZ_PATHS]
    ...
    n_ok = sum(
        1
        for s in per_file
        if s.get("meta_flat", {}).get("status") == "ok" and s.get("derived", {}).get("proxy_slice_ok")
    )
    n_err = sum(1 for s in per_file if s.get("meta_flat", {}).get("status") == "error")
    dataset_quality = {
        "n_paths": len(per_file),
        "n_text_processor_ok_with_proxy": n_ok,
        "n_text_processor_error_or_empty_proxy": len(per_file) - n_ok,
        "n_meta_error": n_err,
        "note": "L2 по смыслу плана требует 5 OK прогонов; при частичном text_processor корреляции и перцентили по 5 строкам матрицы искажены.",
    }
    names, mat = _stack_proxy(per_file)
    ...
    row_ok = np.array(
        [
            s.get("meta_flat", {}).get("status") == "ok" and s.get("derived", {}).get("proxy_slice_ok")
            for s in per_file
        ],
        dtype=bool,
    )
    aggregate = _aggregate_for_rows(np.ones(len(per_file), dtype=bool))
    aggregate["n_runs"] = len(per_file)
    aggregate_ok = _aggregate_for_rows(row_ok) if row_ok.any() else None
    if aggregate_ok is not None:
        aggregate_ok["label"] = "text_processor ok + полный срез tp_asrproxy_*"
    ...
    report = {
        ...
        "per_file": per_file,
        "aggregate": aggregate,
        "aggregate_ok_subset": aggregate_ok,
        "correlation_tabular": {
            "features": names,
            "matrix": corr.tolist() if corr.size else [],
            "low_sample_warning": len(per_file) < 3 or n_ok < 3,
        },
    }
    json_path = args.out_dir / "asr_text_proxy_audio_features_audit_v4_stats.json"
    ...
    if not args.no_plots and len(names) and mat.shape[0] >= 2 and n_ok >= 3:
        try:
            _try_plots(names, mat, args.out_dir, max_features=args.max_plot_features)
            import matplotlib

            matplotlib.use("Agg")
            import matplotlib.pyplot as plt

            fig, ax = plt.subplots(figsize=(max(6, len(names) * 0.32), max(5, len(names) * 0.32)))
            im = ax.imshow(corr, vmin=-1, vmax=1, cmap="RdBu_r")
            ax.set_xticks(range(len(names)))
            ax.set_yticks(range(len(names)))
            ax.set_xticklabels([n.replace("tp_asrproxy_", "")[:14] for n in names], rotation=90, fontsize=6)
            ax.set_yticklabels([n.replace("tp_asrproxy_", "")[:14] for n in names], fontsize=6)
            fig.colorbar(im, ax=ax, fraction=0.046)
            ax.set_title("Pearson corr: tp_asrproxy_* across runs")
            plt.tight_layout()
            (args.out_dir / "figures").mkdir(parents=True, exist_ok=True)
            plt.savefig(args.out_dir / "figures" / "tabular_corr_heatmap.png", dpi=140)
            plt.close(fig)
        except ImportError:
            print("matplotlib not installed; skipped plots", file=sys.stderr)
    elif not args.no_plots and n_ok < 3:
        print("Skipped plots: fewer than 3 OK text_processor runs with tp_asrproxy slice", file=sys.stderr)
