...
Экстрактор возвращает `result.features_flat` (только числовые скаляры):
...
Параметры (prod):
- `comments_mode`: `"aggregates"` (default) или `"matrix"`
- `transcript_source_priority`: list/CSV источников (default `"combined,whisper,youtube_auto"`)
- `compute_*`: feature-gating для отдельных метрик
- `require_any_metric`: fail-fast если все `compute_*` выключены
- `require_title`, `require_description`, `require_transcript`: fail-fast если обязательный вход отсутствует
- `require_comments_for_tc`: fail-fast если включены transcript↔comments метрики, но комменты отсутствуют
- `emit_extra_metrics`: включает дополнительные метрики/тайминги (дороже только в matrix режиме)
