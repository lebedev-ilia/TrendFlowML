#!/usr/bin/env python3
"""
Audit v4 / 4.2: статистика по срезу `tp_cos_*` в `text_processor/text_features.npz`.

Для CosineMetricsExtractor нет отдельных `.npy` артефактов: это чисто табличные скаляры,
построенные на основе downstream relpath'ов эмбеддингов (tp_artifacts).

Скрипт:
  - читает feature_names/feature_values;
  - выделяет только `tp_cos_*` (контракт cosine_metrics_extractor_output_v1);
  - фиксирует качество набора (meta.status ok/error) и наличие полного среза.

Пример:
  cd DataProcessor/TextProcessor
  ../.data_venv/bin/python \
    src/extractors/cosine_metrics_extractor/scripts/audit_v4_npz_stats.py \
    --out-dir ../../storage/audit_v4/cosine_metrics_extractor_l2 \
    --seed 0
"""
...
def summarize_text_npz(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        out["derived"] = {
            ...
            "tp_cos_title_present": float(out["tabular_slice"]["pairwise"].get("tp_cos_title_present"))
            if "tp_cos_title_present" in out["tabular_slice"]["pairwise"]
            else None,
        }
    finally:
        z.close()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 stats: tp_cos_* slice in text_features.npz")
    ...
    json_path = args.out_dir / "cosine_metrics_extractor_audit_v4_stats.json"
    json_path.write_text(json.dumps(_sanitize_json(report), ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {json_path}", file=sys.stderr)
    if n_ok < 3:
        print("Skipped plots: fewer than 3 OK text_processor runs with tp_cos slice", file=sys.stderr)
    return 0
