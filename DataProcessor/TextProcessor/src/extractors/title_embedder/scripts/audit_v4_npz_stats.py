#!/usr/bin/env python3
"""
Audit v4 / 4.2: статистика по `title_embedder`.

Проверяет:
  - табличный срез `tp_titleemb_*` (16 ключей) в `text_processor/text_features.npz`;
  - артефакт `text_processor/_artifacts/title_embedding.npy` (shape/dtype/L2/finite + согласование с `tp_titleemb_dim`).
...
PREFIX = "tp_titleemb_"
EXPECTED_KEYS = 16
ARTIFACT_NAME = "title_embedding.npy"
...
def summarize_text_npz(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        dim_tab = pair.get("tp_titleemb_dim")
        dim_tab_i = None
        if isinstance(dim_tab, (int, float, np.integer, np.floating)) and np.isfinite(float(dim_tab)):
            d = float(dim_tab)
            if float(int(d)) == d:
                dim_tab_i = int(d)
        ...
        out["derived"] = {
            ...
            "tp_titleemb_present": pair.get("tp_titleemb_present"),
        }
        out["consistency"] = {
            "tp_titleemb_dim": dim_tab_i,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 stats: title_embedder (tp_titleemb_* + title_embedding.npy)")
    ...
    json_path = args.out_dir / "title_embedder_audit_v4_stats.json"
    json_path.write_text(json.dumps(_sanitize_json(report), ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {json_path}", file=sys.stderr)
    if n_ok < 3:
        print("Skipped plots: fewer than 3 OK text_processor runs with tp_titleemb slice", file=sys.stderr)
    return 0
