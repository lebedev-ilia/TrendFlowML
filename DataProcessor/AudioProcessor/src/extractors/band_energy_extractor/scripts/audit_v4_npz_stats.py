#!/usr/bin/env python3
"""
Статистика по NPZ `band_energy_extractor` для Audit v4 / 4.2 (план §4).
...
Пример:
  ...
  ../.data_venv/bin/python src/extractors/band_energy_extractor/scripts/audit_v4_npz_stats.py \\
    --result-store ../../../../storage/result_store \\
    --out-dir ../../../../storage/audit_v4/band_energy_extractor_l2
"""
...
def summarize_npz(path: Path) -> Dict[str, Any]:
    ...
    with np.load(path, allow_pickle=True) as data:
        ...
        if "band_energy_shares" in data.files:
            sh = np.asarray(data["band_energy_shares"], dtype=np.float64).ravel()
            if sh.size:
                s = float(np.sum(sh[np.isfinite(sh)]))
                out["derived"]["band_energy_shares_sum"] = s
                out["derived"]["band_energy_shares_sum_deviation_from_1"] = float(abs(s - 1.0))
...
def _round_floats(obj: Any, *, ndigits: int) -> Any:
    ...
    if isinstance(obj, np.ndarray):
        return _round_floats(obj.tolist(), ndigits=ndigits)
    return obj


def golden_signature(summary: Mapping[str, Any], *, ndigits: int = 8) -> Dict[str, Any]:
    """
    Stable-ish signature for §4.8 (reference run A).

    Notes:
    - Not a cryptographic proof of equality, just a compact, reviewable snapshot.
    - Floats are rounded to keep diffs readable.
    """
    meta = summary.get("meta_flat", {}) if isinstance(summary.get("meta_flat"), dict) else {}
    tab = summary.get("tabular", {}) if isinstance(summary.get("tabular"), dict) else {}
    sig = {
        "component": "band_energy_extractor",
        "schema_version": meta.get("schema_version"),
        "producer_version": meta.get("producer_version"),
        "status": meta.get("status"),
        "tabular_pairwise": tab.get("pairwise", {}),
        "derived": summary.get("derived", {}),
        "arrays": {},
    }

    # Keep a small, explicit subset from arrays for golden checks.
    for a in summary.get("arrays", []) if isinstance(summary.get("arrays"), list) else []:
        if not isinstance(a, dict):
            continue
        key = a.get("key")
        if key in ("band_edges_hz", "band_energy_shares"):
            sig["arrays"][str(key)] = {
                "dtype": a.get("dtype"),
                "shape": a.get("shape"),
                "finite_stats": a.get("finite_stats"),
                "nan_frac": a.get("nan_frac"),
                "inf_frac": a.get("inf_frac"),
                "zero_frac": a.get("zero_frac"),
            }

    sig = _round_floats(sig, ndigits=ndigits)
    payload = json.dumps(sig, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
...
def _try_plot_histograms(
    ...
    *,
    ...
    for j, name in enumerate(names):
        ...
        plt.hist(col, bins=min(20, max(5, col.size)), color="#553c9a", edgecolor="white")
        plt.title(name[:80])
        plt.xlabel("value")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 NPZ stats for band_energy_extractor")
    p.add_argument("--result-store", type=Path, default=None)
    p.add_argument("--platform", default="youtube")
    p.add_argument("--component", default="band_energy_extractor")
    p.add_argument("--npz-name", default="band_energy_extractor_features.npz")
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Путь к reference NPZ (набор A) для генерации §4.8 golden-сигнатуры (без сканирования).",
    )
    p.add_argument(
        "--golden-out",
        type=Path,
        default=None,
        help="Куда записать golden JSON (например storage/audit_v4/.../golden_A.json).",
    )
    p.add_argument("--golden-round", type=int, default=8, help="Округление float в golden (для читаемого diff).")
    ...
    json_path = args.out_dir / "band_energy_extractor_audit_v4_stats.json"
    ...
    # Optional: §4.8 golden for reference run A (no execution here; just serializes a signature).
    if args.golden_npz and args.golden_out:
        s = summarize_npz(Path(args.golden_npz))
        g = golden_signature(s, ndigits=int(args.golden_round))
        args.golden_out.parent.mkdir(parents=True, exist_ok=True)
        args.golden_out.write_text(json.dumps(_sanitize_json(g), ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"Wrote golden {args.golden_out}", file=sys.stderr)
