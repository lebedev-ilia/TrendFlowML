#!/usr/bin/env python3
"""Валидатор voice_quality_extractor: схема NPZ, --struct (N + per-segment), --qa (meta + tabular)."""
...
_PER_SEG = ("jitter_by_segment", "shimmer_by_segment", "hnr_by_segment")
...
def validate_schema(npz_path: str) -> bool:
    try:
        ...
        return "voice_quality_extractor_npz_v1" in sv or "voice_quality_extractor" in sv
    except Exception:
        return False
...
def validate_structure(npz_path: str) -> List[str]:
    ...
    for k in _PER_SEG:
        a = d.get(k)
        if a is not None and int(np.asarray(a).size) not in (0, n0):
            out.append(f"{k}: длина {int(np.asarray(a).size)} — ожидается 0 или N={n0}")
    fn, fv = d.get("feature_names"), d.get("feature_values")
    if fn is not None and fv is not None:
        try:
            names = [str(x) for x in np.asarray(fn, dtype=object).ravel()]
...
def main() -> int:
    p = argparse.ArgumentParser(description="validate voice_quality_extractor NPZ")
    ...
    p.add_argument(
        "--struct",
        action="store_true",
        help="Согласованность N и per-segment массивов.",
    )
    args = p.parse_args()
