...
def is_voice_quality_resource_profile_enabled() -> bool:
    v = os.getenv("AP_VOICE_QUALITY_RESOURCE_PROFILE", "")
    return v.strip() in ("1", "true", "True", "yes", "on", "ON")


def capture_voice_quality_resource_profile(*, stage: str) -> Optional[Dict[str, Any]]:
    if not is_voice_quality_resource_profile_enabled():
        return None
