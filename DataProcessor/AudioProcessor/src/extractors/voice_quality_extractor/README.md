...
- Это позволяет использовать более точные методы оценки f0 (PYIN, torchcrepe) из `pitch_extractor`
...
Все фичи контролируются отдельными флагами (opt-in, по умолчанию все выключены):

- `--voice-quality-enable-jitter`: включить jitter метрики
- `--voice-quality-enable-shimmer`: включить shimmer метрики
- `--voice-quality-enable-hnr`: включить HNR метрики
