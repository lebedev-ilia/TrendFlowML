#!/usr/bin/env python3
"""
Статистика по NPZ `voice_quality_extractor` для Audit v4 / 4.2 (план §4).
"""
...
def _parse_meta(meta_obj: Any) -> Dict[str, Any]:
    ...
    for k in (
        ...
        "f0_method",
        "features_enabled",
        "voice_quality_contract_version",
    ):
        if k in meta_obj:
            flat[k] = meta_obj[k]
    return flat
...
def summarize_npz(path: Path) -> Dict[str, Any]:
    ...
    with np.load(path, allow_pickle=True) as data:
        ...
        # Derived: canonical axis sanity (jitter/shimmer/hnr should have same N)
        seg_n: Dict[str, int] = {}
        for key in ("jitter_by_segment", "shimmer_by_segment", "hnr_by_segment"):
            if key in data.files:
                seg_n[key] = int(np.asarray(data[key]).reshape(-1).shape[0])
        if seg_n:
            out["derived"]["segment_arrays_n"] = seg_n
            out["derived"]["segment_arrays_all_equal"] = bool(len(set(seg_n.values())) == 1)
...
def _try_plot_histograms(names: List[str], mat: np.ndarray, out_dir: Path, *, max_features: int) -> None:
    ...
    for j, name in enumerate(names):
        ...
        plt.hist(col, bins=min(24, max(6, col.size)), color="#22c55e", edgecolor="white")
        plt.title(name[:80])
        plt.tight_layout()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 NPZ stats for voice_quality_extractor")
    p.add_argument("--result-store", type=Path, default=None)
    p.add_argument("--platform", default="youtube")
    p.add_argument("--component", default="voice_quality_extractor")
    p.add_argument("--npz-name", default="voice_quality_extractor_features.npz")
    ...
    json_path = args.out_dir / "voice_quality_extractor_audit_v4_stats.json"
