...
**Энергетические метрики** (feature-gated: `--hpss-enable-energy-metrics`):
...
Все фичи управляются отдельными флагами (opt-in, по умолчанию все выключены):

- `--hpss-enable-energy-metrics`: энергетические метрики (shares, energies, stability, separation quality, balance score, dominance)
...
1. **Загрузка аудио**: через `AudioUtils.load_audio()` или `AudioUtils.load_audio_segment()` (для `run_segments()`) с ресемплированием до `sample_rate`
