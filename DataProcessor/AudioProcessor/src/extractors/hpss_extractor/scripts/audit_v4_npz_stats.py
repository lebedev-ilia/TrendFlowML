#!/usr/bin/env python3
"""
Статистика по NPZ `hpss_extractor` для Audit v4 / 4.2 (план §4).

Сводка по файлам, агрегат по прогонам, корреляции tabular, опционально PNG.
Учитывает, что набор tabular может зависеть от `meta.features_enabled`.
"""
...
def summarize_npz(path: Path) -> Dict[str, Any]:
    ...
    with np.load(path, allow_pickle=True) as data:
        ...
        for k in ("hpss_harmonic_share_by_segment", "hpss_percussive_share_by_segment"):
            if k in data.files:
...
def _try_plot_histograms(names: List[str], mat: np.ndarray, out_dir: Path, *, max_features: int) -> None:
    ...
    for j, name in enumerate(names):
        ...
        plt.hist(col, bins=min(24, max(6, col.size)), color="#16a34a", edgecolor="white")
        plt.title(name[:80])
        plt.tight_layout()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 NPZ stats for hpss_extractor")
    p.add_argument("--result-store", type=Path, default=None)
    p.add_argument("--platform", default="youtube")
    p.add_argument("--component", default="hpss_extractor")
    p.add_argument("--npz-name", default="hpss_extractor_features.npz")
    ...
    json_path = args.out_dir / "hpss_extractor_audit_v4_stats.json"
