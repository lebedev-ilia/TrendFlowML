...
- `source_distribution`: распределение времени по источникам (dict[source_id, time_ratio])
- `source_segments_per_source`: количество сегментов для каждого источника (где источник доминирует) (dict[source_id, count])
- `source_duration_per_source`: длительность каждого источника в секундах (dict[source_id, duration_sec])
...
- `source_quality_metrics`: метрики качества разделения:
  ```python
  {
      "share_mean_min": float,      # Минимальная средняя доля
      "share_mean_max": float,      # Максимальная средняя доля
      "share_mean_std": float,      # Стандартное отклонение средних долей
      "share_std_mean": float,      # Среднее стандартное отклонение долей
      "share_std_max": float,       # Максимальное стандартное отклонение долей
      "share_sequence_min": float,   # Минимальная доля в последовательности
      "share_sequence_max": float,   # Максимальная доля в последовательности
      "share_sequence_mean": float,  # Средняя доля в последовательности
      "energy_sequence_min": float,  # Минимальная энергия
      "energy_sequence_max": float,  # Максимальная энергия
      "energy_sequence_mean": float, # Средняя энергия
  }
  ```
...
- `source_transitions_count`, `source_distribution`, `source_segments_per_source`, `source_duration_per_source`, `source_stability_score` зависят от `share_sequence` (используют его для вычисления transitions и distribution)
...
3. Вычисление дополнительных агрегатов (feature-gated, требует `enable_share_sequence`):
   ...
   - `source_distribution`: доли времени по источникам
   - `source_segments_per_source`: количество сегментов для каждого источника
   - `source_duration_per_source`: длительность каждого источника в секундах
...
- `--sep-enable-share-mean`: включить `share_mean` (mean shares)
- `--sep-enable-share-std`: включить `share_std` (std shares)
- `--sep-enable-quality-metrics`: включить `source_quality_metrics` (метрики качества)
...
2. **Распределение источников**:
   - Pie chart или bar chart с долями времени по источникам (`source_distribution`)
...
10. **Feature gating**: все фичи opt-in через персональные флаги. Для обучения рекомендуется включить все фичи для максимального качества.
