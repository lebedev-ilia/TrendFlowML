#!/usr/bin/env python3
"""
Статистика по NPZ `source_separation_extractor` для Audit v4 / 4.2 (план §4).
...
def _parse_meta(meta_obj: Any) -> Dict[str, Any]:
    ...
    for k in (
        ...
        "model_name",
        "weights_digest",
        "source_separation_contract_version",
    ):
        if k in meta_obj:
            flat[k] = meta_obj[k]
    return flat
...
def summarize_npz(path: Path) -> Dict[str, Any]:
    ...
    with np.load(path, allow_pickle=True) as data:
        ...
        if "share_mean" in data.files:
            sm = np.asarray(data["share_mean"]).astype(np.float64).reshape(-1)
            if sm.size == 4 and np.isfinite(sm).all():
                out["derived"]["share_mean_sum"] = float(sm.sum())
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 NPZ stats for source_separation_extractor")
    p.add_argument("--result-store", type=Path, default=None)
    p.add_argument("--platform", default="youtube")
    p.add_argument("--component", default="source_separation_extractor")
    p.add_argument("--npz-name", default="source_separation_extractor_features.npz")
    ...
    json_path = args.out_dir / "source_separation_extractor_audit_v4_stats.json"
