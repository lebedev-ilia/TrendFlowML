...
def is_source_separation_resource_profile_enabled() -> bool:
    v = os.getenv("AP_SOURCE_SEPARATION_RESOURCE_PROFILE", "")
    return v.strip() in ("1", "true", "True", "yes", "on", "ON")


def capture_source_separation_resource_profile(*, stage: str) -> Optional[Dict[str, Any]]:
    if not is_source_separation_resource_profile_enabled():
        return None
