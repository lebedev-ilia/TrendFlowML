#!/usr/bin/env python3
"""Валидатор source_separation_extractor: схема, --struct (N, optional [N,4] seq), --qa (meta + tabular)."""
from __future__ import annotations

import argparse
import sys
from pathlib import Path
from typing import Any, Dict, List, Tuple

import numpy as np

_SEGMENT = (
    "segment_start_sec",
    "segment_end_sec",
    "segment_center_sec",
    "segment_mask",
)
_VEC4 = ("source_distribution_ratio", "source_segments_count", "source_duration_sec")
_N_CLASS = 4


def load_npz(npz_path: str) -> Dict[str, Any]:
    data = np.load(npz_path, allow_pickle=True)
    return {
        k: (data[k].item() if data[k].dtype == object and data[k].size == 1 else data[k])
        for k in data.files
    }


def extract_meta(d: Dict) -> Dict:
    m = d.get("meta")
    return m.item() if hasattr(m, "item") else (m or {})


def validate_schema(npz_path: str) -> bool:
    try:
        d = load_npz(npz_path)
        if "meta" not in d:
            return False
        meta = extract_meta(d)
        sv = str(meta.get("schema_version", ""))
        return "source_separation_extractor_npz_v2" in sv or "source_separation_extractor" in sv
    except Exception:
        return False


def _dataprocessor_on_path() -> Path:
    return Path(__file__).resolve().parents[5]


def _load_qa_config() -> Tuple[Any, Path]:
    dp = _dataprocessor_on_path()
    r = str(dp)
    if r not in sys.path:
        sys.path.insert(0, r)
    from qa.component_feature_qa import find_repo_root_from_path, load_qa_config

    root = find_repo_root_from_path(Path(__file__))
    if root is None:
        raise FileNotFoundError("view_csv_feature_qa.json (repo root not found)")
    path = root / "storage" / "result_store" / "view_csv_feature_qa.json"
    if not path.is_file():
        raise FileNotFoundError(str(path))
    return load_qa_config(path), path


def validate_qa_rows(npz_path: str, qa: Any) -> List[str]:
    from qa.component_feature_qa import flatten_meta

    d = load_npz(npz_path)
    meta = extract_meta(d)
    flat: Dict[str, Any] = dict(flatten_meta(meta, prefix="meta_"))
    fn, fv = d.get("feature_names"), d.get("feature_values")
    if fn is not None and fv is not None:
        try:
            names = [str(x) for x in np.asarray(fn, dtype=object).ravel()]
            vals = np.asarray(fv, dtype=np.float64).ravel()
            for n, v in zip(names, vals):
                flat[str(n)] = v
        except Exception:
            pass
    warnings: List[str] = []
    comp = "source_separation_extractor"
    rmap = qa.rules_for_column(comp)
    for col in sorted(rmap.keys()):
        if col not in flat:
            continue
        raw = str(flat[col])
        w = qa.warning_for(comp, col, raw)
        if w:
            warnings.append(f"{col}: {w}")
    return warnings


def validate_structure(npz_path: str) -> List[str]:
    out: List[str] = []
    d = load_npz(npz_path)
    miss = [k for k in _SEGMENT if k not in d]
    if miss:
        return [f"нет сегментных ключей: {miss}"]
    n0 = int(np.asarray(d["segment_start_sec"]).size)
    for k in _SEGMENT:
        if int(np.asarray(d[k]).size) != n0:
            out.append(f"{k}: длина != N={n0}")
    sm = d.get("share_mean")
    if sm is not None and int(np.asarray(sm).ravel().size) != _N_CLASS:
        out.append(f"share_mean: ожидается длина {_N_CLASS}, got {int(np.asarray(sm).ravel().size)}")
    for k in _VEC4:
        a = d.get(k)
        if a is not None and int(np.asarray(a).ravel().size) != _N_CLASS:
            out.append(f"{k}: ожидается длина {_N_CLASS}")
    so = d.get("source_order")
    if so is not None and int(np.asarray(so, dtype=object).ravel().size) != _N_CLASS:
        out.append("source_order: ожидается 4 элемента")
    ss = d.get("share_sequence")
    if ss is not None:
        a = np.asarray(ss, dtype=np.float32)
        if a.ndim != 2 or a.shape[1] != _N_CLASS:
            out.append("share_sequence: ожидается [N,4]")
        elif a.shape[0] != n0:
            out.append(f"share_sequence: axis0 {a.shape[0]} != N={n0}")
    es = d.get("energy_sequence")
    if es is not None:
        a = np.asarray(es, dtype=np.float32)
        if a.ndim != 2 or a.shape[1] != _N_CLASS:
            out.append("energy_sequence: ожидается [N,4]")
        elif a.shape[0] != n0:
            out.append(f"energy_sequence: axis0 {a.shape[0]} != N={n0}")
    sstd = d.get("share_std")
    if sstd is not None and int(np.asarray(sstd).ravel().size) != _N_CLASS:
        out.append("share_std: ожидается [4]")
    fn, fv = d.get("feature_names"), d.get("feature_values")
    if fn is not None and fv is not None:
        try:
            names = [str(x) for x in np.asarray(fn, dtype=object).ravel()]
            vals = np.asarray(fv, dtype=np.float64).ravel()
            if len(names) != len(vals):
                out.append("feature_names/feature_values: несовпадение длины")
        except Exception as e:  # pragma: no cover
            out.append(f"tabular: {e}")
    return out


def main() -> int:
    p = argparse.ArgumentParser(description="validate source_separation_extractor NPZ")
    p.add_argument("npz_path")
    p.add_argument(
        "--qa",
        action="store_true",
        help="Проверить плоский meta + tabular (view_csv_feature_qa.json).",
    )
    p.add_argument(
        "--struct",
        action="store_true",
        help="Согласованность N, векторов [4] и [N,4] последовательностей.",
    )
    args = p.parse_args()
    ok = validate_schema(args.npz_path)
    print("✅ VALID schema" if ok else "❌ INVALID schema")
    if not ok:
        return 1
    ex = 0
    if args.struct:
        st = validate_structure(args.npz_path)
        if st:
            print("⚠️  structure:")
            for s in st:
                print("  -", s)
            ex = max(ex, 2)
    if args.qa:
        try:
            qa, path = _load_qa_config()
        except Exception as e:
            print(f"QA: пропуск ({e})", flush=True)
            return ex or 0
        warns = validate_qa_rows(args.npz_path, qa)
        if warns:
            print(f"⚠️  QA warnings ({path}):")
            for w in warns:
                print("  -", w)
            ex = max(ex, 2)
        else:
            print(f"✅ QA OK (rules {path})")
    return ex


if __name__ == "__main__":
    sys.exit(main())
