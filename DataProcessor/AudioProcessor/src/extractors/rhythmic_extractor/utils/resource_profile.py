...
def is_rhythmic_resource_profile_enabled() -> bool:
    v = os.getenv("AP_RHYTHMIC_RESOURCE_PROFILE", "")
    return v.strip() in ("1", "true", "True", "yes", "on", "ON")


def capture_rhythmic_resource_profile(*, stage: str) -> Optional[Dict[str, Any]]:
    if not is_rhythmic_resource_profile_enabled():
        return None
