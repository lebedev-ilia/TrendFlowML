#!/usr/bin/env python3
"""Валидатор rhythmic_extractor: схема, --struct (N, beats, tabular), --qa (плоский meta)."""
...
def validate_schema(npz_path: str) -> bool:
    try:
        ...
        return "rhythmic_extractor_npz_v2" in sv or "rhythmic_extractor" in sv
    except Exception:
        return False
...
def validate_structure(npz_path: str) -> List[str]:
    ...
    bt = d.get("beat_times_sec")
    bi = d.get("beat_segment_index")
    if bt is not None and int(np.asarray(bt).size) > 0:
        m_bt = int(np.asarray(bt, dtype=np.float64).size)
        if bi is not None and int(np.asarray(bi).size) > 0:
            m_bi = int(np.asarray(bi).size)
            if m_bt != m_bi:
                out.append(f"beat_times_sec ({m_bt}) != beat_segment_index ({m_bi})")
    elif bi is not None and int(np.asarray(bi).size) > 0 and (
        bt is None or int(np.asarray(bt).size) == 0
    ):
        out.append("beat_segment_index задан, а beat_times_sec пуст/отсутствует")
    ...
    if fn is not None and fv is not None:
        try:
            ...
            elif not (6 <= len(names) <= 24):
                out.append(
                    f"tabular: неожиданное число фич (замороженный вектор ~9 + контекст, facts={len(names)})"
                )
...
def main() -> int:
    p = argparse.ArgumentParser(description="validate rhythmic_extractor NPZ")
    p.add_argument("npz_path")
    p.add_argument(
        "--qa",
        action="store_true",
        help="Проверить плоский meta (view_csv_feature_qa.json → rhythmic_extractor).",
    )
    p.add_argument(
        "--struct",
        action="store_true",
        help="Сегменты N, пары битов, tabular.",
    )
    args = p.parse_args()
