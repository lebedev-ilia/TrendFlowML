#!/usr/bin/env python3
"""
Статистика по NPZ `rhythmic_extractor` для Audit v4 / 4.2 (план §4).
...
def _parse_meta(meta_obj: Any) -> Dict[str, Any]:
    ...
    for k in (
        ...
        "empty_reason",
        "backend",
        "sampling_family_used",
        "rhythmic_contract_version",
        "features_enabled",
    ):
        if k in meta_obj:
            flat[k] = meta_obj[k]
...
def summarize_npz(path: Path) -> Dict[str, Any]:
    ...
    with np.load(path, allow_pickle=True) as data:
        ...
        if "beat_times_sec" in data.files:
            b = np.asarray(data["beat_times_sec"]).astype(np.float64).ravel()
            out["derived"]["beats_count"] = int(b.size)
            out["derived"]["beat_times_sec_nan_frac"] = float(np.isnan(b).sum() / max(b.size, 1))
...
def _try_plot_histograms(names: List[str], mat: np.ndarray, out_dir: Path, *, max_features: int) -> None:
    ...
    for j, name in enumerate(names):
        ...
        plt.hist(col, bins=min(24, max(6, col.size)), color="#14b8a6", edgecolor="white")
        plt.title(name[:80])
        plt.tight_layout()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 NPZ stats for rhythmic_extractor")
    p.add_argument("--result-store", type=Path, default=None)
    p.add_argument("--platform", default="youtube")
    p.add_argument("--component", default="rhythmic_extractor")
    p.add_argument("--npz-name", default="rhythmic_extractor_features.npz")
    ...
    json_path = args.out_dir / "rhythmic_extractor_audit_v4_stats.json"
