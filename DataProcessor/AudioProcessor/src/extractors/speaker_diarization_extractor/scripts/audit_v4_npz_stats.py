#!/usr/bin/env python3
"""
Статистика по NPZ `speaker_diarization_extractor` для Audit v4 / 4.2 (план §4).
...
def summarize_npz(path: Path) -> Dict[str, Any]:
    ...
    with np.load(path, allow_pickle=True) as data:
        ...
        if "turn_speaker_id" in data.files:
            tsi = np.asarray(data["turn_speaker_id"]).reshape(-1)
            out["derived"]["turns_count"] = int(tsi.size)
        if "speaker_ids" in data.files:
            sid = np.asarray(data["speaker_ids"]).reshape(-1)
            out["derived"]["speakers_count_arr"] = int(sid.size)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 NPZ stats for speaker_diarization_extractor")
    p.add_argument("--result-store", type=Path, default=None)
    p.add_argument("--platform", default="youtube")
    p.add_argument("--component", default="speaker_diarization_extractor")
    p.add_argument("--npz-name", default="speaker_diarization_extractor_features.npz")
    ...
    json_path = args.out_dir / "speaker_diarization_extractor_audit_v4_stats.json"
