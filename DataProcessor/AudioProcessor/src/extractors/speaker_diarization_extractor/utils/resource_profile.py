...
def is_speaker_diarization_resource_profile_enabled() -> bool:
    v = os.getenv("AP_SPEAKER_DIARIZATION_RESOURCE_PROFILE", "")
    return v.strip() in ("1", "true", "True", "yes", "on", "ON")


def capture_speaker_diarization_resource_profile(*, stage: str) -> Optional[Dict[str, Any]]:
    if not is_speaker_diarization_resource_profile_enabled():
        return None
