#!/usr/bin/env python3
"""
Статистика по NPZ `spectral_entropy_extractor` для Audit v4 / 4.2 (план §4).
"""
...
def _try_plot_histograms(names: List[str], mat: np.ndarray, out_dir: Path, *, max_features: int) -> None:
    ...
    for j, name in enumerate(names):
        ...
        plt.hist(col, bins=min(24, max(6, col.size)), color="#a855f7", edgecolor="white")
        plt.title(name[:80])
        plt.tight_layout()
...
def _try_plot_corr_heatmap(names: List[str], corr: np.ndarray, out_path: Path) -> None:
    ...
    if F == 0:
        return
    fig, ax = plt.subplots(figsize=(max(7, F * 0.6), max(6, F * 0.6)))
    im = ax.imshow(corr, vmin=-1, vmax=1, cmap="RdBu_r")
    ax.set_xticks(range(F))
    ax.set_yticks(range(F))
    ax.set_xticklabels([n[:14] for n in names], rotation=90, fontsize=8)
    ax.set_yticklabels([n[:14] for n in names], fontsize=8)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 NPZ stats for spectral_entropy_extractor")
    p.add_argument("--result-store", type=Path, default=None)
    p.add_argument("--platform", default="youtube")
    p.add_argument("--component", default="spectral_entropy_extractor")
    p.add_argument("--npz-name", default="spectral_entropy_extractor_features.npz")
    ...
    json_path = args.out_dir / "spectral_entropy_extractor_audit_v4_stats.json"
