#!/usr/bin/env python3
"""Валидатор spectral_entropy_extractor: схема, --struct (N, per-segment), --qa (плоский meta)."""
...
_REQUIRED_PER_SEG = ("entropy_mean_by_segment", "entropy_std_by_segment")
_OPTIONAL_PER_SEG = (
    "entropy_min_by_segment",
    "entropy_max_by_segment",
    "flatness_mean_by_segment",
    "flatness_std_by_segment",
    "spread_mean_by_segment",
    "spread_std_by_segment",
)
...
def validate_schema(npz_path: str) -> bool:
    try:
        ...
        return "spectral_entropy_extractor_npz_v2" in sv or "spectral_entropy_extractor" in sv
    except Exception:
        return False
...
def validate_qa_rows(npz_path: str, qa: Any) -> List[str]:
    ...
    warnings: List[str] = []
    comp = "spectral_entropy_extractor"
...
def validate_structure(npz_path: str) -> List[str]:
    ...
    for k in _REQUIRED_PER_SEG:
        if k not in d:
            out.append(f"нет обязательного {k}")
        elif int(np.asarray(d[k]).size) != n0:
            out.append(f"{k}: длина {int(np.asarray(d[k]).size)} != N={n0}")
    for k in _OPTIONAL_PER_SEG:
        a = d.get(k)
...
def main() -> int:
    p = argparse.ArgumentParser(description="validate spectral_entropy_extractor NPZ")
    p.add_argument("npz_path")
    p.add_argument(
        "--qa",
        action="store_true",
        help="Проверить плоский meta (view_csv_feature_qa.json → spectral_entropy_extractor).",
    )
    p.add_argument(
        "--struct",
        action="store_true",
