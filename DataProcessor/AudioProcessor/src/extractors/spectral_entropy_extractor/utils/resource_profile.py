...
def is_spectral_entropy_resource_profile_enabled() -> bool:
    v = os.getenv("AP_SPECTRAL_ENTROPY_RESOURCE_PROFILE", "")
    return v.strip() in ("1", "true", "True", "yes", "on", "ON")


def capture_spectral_entropy_resource_profile(*, stage: str) -> Optional[Dict[str, Any]]:
    if not is_spectral_entropy_resource_profile_enabled():
        return None
