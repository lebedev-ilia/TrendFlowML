#!/usr/bin/env python3
"""
Статистика по NPZ `onset_extractor` для Audit v4 / 4.2 (план §4).
...
def _parse_meta(meta_obj: Any) -> Dict[str, Any]:
    if not isinstance(meta_obj, dict):
        return {}
    flat: Dict[str, Any] = {}
    for k in ("schema_version", "status", "producer_version", "empty_reason", "backend", "onset_contract_version"):
        if k in meta_obj:
            flat[k] = meta_obj[k]
    return flat
...
def _try_plot_histograms(names: List[str], mat: np.ndarray, out_dir: Path, *, max_features: int) -> None:
    ...
    for j, name in enumerate(names):
        ...
        plt.hist(col, bins=min(24, max(6, col.size)), color="#f97316", edgecolor="white")
        plt.title(name[:80])
        plt.tight_layout()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 NPZ stats for onset_extractor")
    p.add_argument("--result-store", type=Path, default=None)
    p.add_argument("--platform", default="youtube")
    p.add_argument("--component", default="onset_extractor")
    p.add_argument("--npz-name", default="onset_extractor_features.npz")
    ...
    json_path = args.out_dir / "onset_extractor_audit_v4_stats.json"
