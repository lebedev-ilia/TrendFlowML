#!/usr/bin/env python3
"""Валидатор onset_extractor: схема, --struct (N, tabular), --qa (meta + tabular)."""
...
def validate_schema(npz_path: str) -> bool:
    try:
        ...
        return "onset_extractor_npz_v2" in sv or "onset_extractor" in sv
    except Exception:
        return False
    ...
    if fn is not None and fv is not None:
        try:
            ...
            if len(names) != len(vals):
                ...
                out.append(f"tabular: слишком много фич (facts={len(names)}), ожидается ≤128")
...
def main() -> int:
    p = argparse.ArgumentParser(description="validate onset_extractor NPZ")
    p.add_argument("npz_path")
    p.add_argument(
        "--qa",
        action="store_true",
        help="Проверить плоский meta (view_csv_feature_qa.json → onset_extractor).",
    )
    p.add_argument(
        "--struct",
        action="store_true",
        help="Согласованность N по сегментам и tabular.",
    )
    args = p.parse_args()
