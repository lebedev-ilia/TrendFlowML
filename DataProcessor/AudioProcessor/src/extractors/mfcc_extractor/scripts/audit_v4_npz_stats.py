#!/usr/bin/env python3
"""
Статистика по NPZ `mfcc_extractor` для Audit v4 / 4.2 (план §4).
...
def summarize_npz(path: Path) -> Dict[str, Any]:
    ...
    with np.load(path, allow_pickle=True) as data:
        ...
        if "mfcc_energy_by_segment" in data.files:
            x = np.asarray(data["mfcc_energy_by_segment"], dtype=np.float64).ravel()
            fin = x[np.isfinite(x)]
            out["derived"]["mfcc_energy_by_segment"] = {
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 NPZ stats for mfcc_extractor")
    p.add_argument("--result-store", type=Path, default=None)
    p.add_argument("--platform", default="youtube")
    p.add_argument("--component", default="mfcc_extractor")
    p.add_argument("--npz-name", default="mfcc_extractor_features.npz")
    ...
    json_path = args.out_dir / "mfcc_extractor_audit_v4_stats.json"
