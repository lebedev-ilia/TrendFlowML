#!/usr/bin/env python3
"""Валидатор mfcc_extractor: схема, --struct (N, M, опц. дельты/series), --qa (плоский meta)."""
...
def validate_schema(npz_path: str) -> bool:
    try:
        ...
        return "mfcc_extractor_npz_v2" in sv or "mfcc_extractor" in sv
    except Exception:
        return False
...
def validate_structure(npz_path: str) -> List[str]:
    ...
    td = _tabular_dict(d)
    m_expected = _n_mfcc_from_tabular(td)

    for key in ("mfcc_mean", "mfcc_std", "mfcc_min", "mfcc_max", "delta_mean", "delta_std", "delta_delta_mean", "delta_delta_std"):
        a = d.get(key)
        if a is not None and np.size(a) > 0:
            ln = int(np.asarray(a).ravel().size)
            if m_expected is not None and ln != m_expected:
                out.append(f"{key}: длина {ln} != n_mfcc={m_expected}")

    mbs = d.get("mfcc_mean_by_segment")
    if mbs is not None and np.size(mbs) > 0:
        mm = np.asarray(mbs, dtype=np.float64)
        if mm.ndim != 2:
            out.append(f"mfcc_mean_by_segment: ожидается 2D, ndim={mm.ndim}")
        else:
            n_r, m_r = int(mm.shape[0]), int(mm.shape[1])
            if n_r != n0:
                out.append(f"mfcc_mean_by_segment: ось0 {n_r} != N={n0}")
            if m_expected is not None and m_r != m_expected:
                out.append(f"mfcc_mean_by_segment: ось1 {m_r} != n_mfcc={m_expected}")

    dms = d.get("delta_mean_by_segment")
    if dms is not None and np.size(dms) > 0:
        dd = np.asarray(dms, dtype=np.float64)
        if dd.ndim == 2:
            if int(dd.shape[0]) != n0:
                out.append(f"delta_mean_by_segment: ось0 {int(dd.shape[0])} != N={n0}")
            if m_expected is not None and int(dd.shape[1]) != m_expected:
                out.append(f"delta_mean_by_segment: ось1 {int(dd.shape[1])} != n_mfcc={m_expected}")

    for skey in ("mfcc_energy_by_segment",):
        a = d.get(skey)
        if a is not None and np.size(a) > 0 and int(np.asarray(a).size) != n0:
    ...
    if fn is not None and fv is not None:
        try:
            ...
            elif not (8 <= len(names) <= 32):
                out.append(
                    f"tabular: неожиданное число фич (ожидается ~13, SCHEMA v2), facts={len(names)}"
                )
...
def main() -> int:
    p = argparse.ArgumentParser(description="validate mfcc_extractor NPZ")
    p.add_argument("npz_path")
    p.add_argument(
        "--qa",
        action="store_true",
        help="Проверить плоский meta (view_csv_feature_qa.json → mfcc_extractor).",
    )
    p.add_argument(
        "--struct",
        action="store_true",
        help="Согласованность N, n_mfcc, опциональные MFCC/дельта массивы.",
    )
    args = p.parse_args()
