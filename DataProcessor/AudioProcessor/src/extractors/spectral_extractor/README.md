...
#### Временные серии (feature-gated: `--spectral-enable-time-series`)

- **`centroid_series`**: временная серия центроида (float32[])
- **`bandwidth_series`**: временная серия bandwidth (float32[])
- **`flatness_series`**: временная серия flatness (float32[])
- **`rolloff_series`**: временная серия rolloff (float32[])
- **`zcr_series`**: временная серия ZCR (float32[])
- **`contrast_series`**: временная серия contrast (float32[], если включен contrast)
- **`slope_series`**: временная серия slope (float32[], если включены advanced_features)
...
4. **Feature gating**: все фичи opt-in (по умолчанию все выключены) для контроля размера NPZ и стоимости вычислений
