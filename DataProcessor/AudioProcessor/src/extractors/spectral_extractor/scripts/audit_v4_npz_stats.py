#!/usr/bin/env python3
"""
Статистика по NPZ `spectral_extractor` для Audit v4 / 4.2 (план §4).
"""
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 NPZ stats for spectral_extractor")
    p.add_argument("--result-store", type=Path, default=None)
    p.add_argument("--platform", default="youtube")
    p.add_argument("--component", default="spectral_extractor")
    p.add_argument("--npz-name", default="spectral_extractor_features.npz")
    ...
    json_path = args.out_dir / "spectral_extractor_audit_v4_stats.json"
