...
def is_speech_analysis_resource_profile_enabled() -> bool:
    v = os.getenv("AP_SPEECH_ANALYSIS_RESOURCE_PROFILE", "")
    return v.strip() in ("1", "true", "True", "yes", "on", "ON")


def capture_speech_analysis_resource_profile(*, stage: str) -> Optional[Dict[str, Any]]:
    if not is_speech_analysis_resource_profile_enabled():
        return None
