#!/usr/bin/env python3
"""
Статистика по NPZ `speech_analysis_extractor` для Audit v4 / 4.2 (план §4).
"""
...
def _parse_meta(meta_obj: Any) -> Dict[str, Any]:
    ...
    for k in (
        ...
        "device_used",
        "features_enabled",
        "speech_analysis_contract_version",
    ):
        if k in meta_obj:
            flat[k] = meta_obj[k]
    return flat
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 NPZ stats for speech_analysis_extractor")
    p.add_argument("--result-store", type=Path, default=None)
    p.add_argument("--platform", default="youtube")
    p.add_argument("--component", default="speech_analysis_extractor")
    p.add_argument("--npz-name", default="speech_analysis_extractor_features.npz")
    ...
    json_path = args.out_dir / "speech_analysis_extractor_audit_v4_stats.json"
