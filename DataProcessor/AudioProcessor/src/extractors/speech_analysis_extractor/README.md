...
4. **Обработка ошибок**:
   ...
   - Audio too short → `RuntimeError` с `error_code="audio_too_short"`
...
- Минимальная длительность аудио: **5 секунд** (иначе ошибка `audio_too_short`)
...
1. **ASR метрики**:
   - Отображение `speech_rate_wpm` (скорость речи)
   - Визуализация `lang_distribution` (pie chart или bar chart)
   - Timeline с `lang_id_by_segment` (цветовая кодировка языков)
   - Статистики: `token_total`, `token_density_per_sec`

2. **Diarization метрики**:
   - Отображение `speaker_count` и `speaker_balance_score`
   - Визуализация `speaker_transitions_count` (индикатор активности)
   - Timeline с `speaker_ids` (цветовая кодировка спикеров)
   - Статистики: `dominant_speaker_share`

3. **Pitch метрики**:
   - Отображение `pitch_f0_mean` и `pitch_stability`
   - Визуализация `pitch_distribution` (bar chart по октавам)
   - Range: `pitch_f0_min` - `pitch_f0_max`

4. **HTML renderer для дебага**:
   - Локальный HTML renderer доступен через `render_speech_analysis_extractor_html()`
   - Включает все метрики, распределения, статистики
   - Только для локального дебага, не в production артефактах

**Пример использования HTML renderer**:
```python
from src.core.renderer import render_speech_analysis_extractor_html
...
2. **Минимальная длительность**: аудио должно быть ≥ 5 секунд
...
5. **Тихое аудио**: автоматически определяется и возвращается как `empty`, а не ошибка (если silence detection включен)
