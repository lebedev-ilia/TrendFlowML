#!/usr/bin/env python3
"""Валидатор quality_extractor: схема, --struct (N, tabular), --qa (плоский meta)."""
...
def validate_schema(npz_path: str) -> bool:
    try:
        ...
        return "quality_extractor_npz_v2" in sv or "quality_extractor" in sv
    except Exception:
        return False
...
def main() -> int:
    p = argparse.ArgumentParser(description="validate quality_extractor NPZ")
    p.add_argument("npz_path")
    p.add_argument(
        "--qa",
        action="store_true",
        help="Проверить плоский meta (view_csv_feature_qa.json → quality_extractor).",
    )
    p.add_argument(
        "--struct",
        action="store_true",
        help="Сегменты N, tabular.",
    )
    args = p.parse_args()
