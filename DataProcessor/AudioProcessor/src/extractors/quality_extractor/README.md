...
- **`audio/segments.json`** (Segmenter contract, family `quality`) — сегменты для `run_segments()`

### Выходы

Экстрактор возвращает `ExtractorResult` с payload, содержащим:

#### Базовые метрики (feature-gated: `--quality-enable-basic-metrics`)
...
- **`dc_offset_abs`**: абсолютное значение DC offset (дополнительная метрика)
  - **Единицы**: нормализованные амплитуды [0.0, 1.0]
...
- **`snr_db`**: грубая оценка отношения сигнал/шум
  - **Интерпретация**: больше = лучше качество сигнала
  - **Единицы**: децибелы (dB)
  - **Метод**: разница между 95-м и 5-м перцентилями уровней кадров
  - **Диапазон**: ≥ 0 dB
  - **Примечание**: грубая оценка, не заменяет профессиональные инструменты измерения SNR
...
- **`dynamic_range_stability`**: стабильность dynamic range (`1.0 / (1.0 + std(dynamic_range_db))`)
- **`snr_stability`**: стабильность SNR (`1.0 / (1.0 + std(snr_db))`)
...
- **`dc_offset_abs`** зависит от **`dc_offset`** (требует включения `--quality-enable-basic-metrics`)
- **`dynamic_range_stability`** зависит от **`dynamic_range_db`** (требует включения `--quality-enable-dynamic-metrics`)
- **`snr_stability`** зависит от **`snr_db`** (требует включения `--quality-enable-dynamic-metrics`)
- **`quality_score`** зависит от всех базовых и динамических метрик (требует включения `--quality-enable-basic-metrics` и `--quality-enable-dynamic-metrics`)
- **`frame_levels_distribution`** зависит от анализа кадров (требует включения `--quality-enable-frame-analysis`)
- **`clipping_segments_series`** зависит от **`clipping_ratio`** (требует включения `--quality-enable-basic-metrics` и `--quality-enable-frame-analysis`)
...
--quality-enable-basic-metrics       # Включить базовые метрики (dc_offset, clipping_ratio, crest_factor_db)
--quality-enable-dynamic-metrics    # Включить динамические метрики (dynamic_range_db, snr_db)
...
4. **Dynamic Range**: разница между 95-м и 5-м перцентилями уровней кадров
5. **SNR**: грубая оценка через перцентили (`max(0.0, signal_db - noise_db)`)
...
- **Ошибка вычисления SNR**: `quality_snr_failed`
...
- Проверка диапазонов значений (например, clipping_ratio ∈ [0, 1], crest_factor_db ≥ 0, dynamic_range_db ≥ 0, snr_db ≥ 0)
- Проверка NaN/inf в значениях
- Проверка консистентности (snr_db ≤ dynamic_range_db)
