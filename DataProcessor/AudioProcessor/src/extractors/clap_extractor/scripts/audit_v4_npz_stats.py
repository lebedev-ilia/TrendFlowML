#!/usr/bin/env python3
"""
Статистика по NPZ `clap_extractor` для Audit v4 / 4.2 (план §4).
...
def summarize_npz(path: Path) -> Dict[str, Any]:
    ...
    with np.load(path, allow_pickle=True) as data:
        ...
        # Derived: norms and validity
        if "embedding" in data.files:
            emb = np.asarray(data["embedding"], dtype=np.float64).ravel()
            fin = emb[np.isfinite(emb)]
            out["derived"]["embedding_dim"] = int(fin.size)
            out["derived"]["embedding_norm"] = float(np.linalg.norm(fin)) if fin.size else 0.0

        if "segment_mask" in data.files:
            m = np.asarray(data["segment_mask"]).astype(bool).ravel()
            out["derived"]["segments_count_total"] = int(m.size)
            out["derived"]["segments_valid"] = int(m.sum())

        if "embedding_sequence" in data.files:
            seq = np.asarray(data["embedding_sequence"], dtype=np.float64)
            if seq.ndim == 2 and seq.size:
                row_ok = np.isfinite(seq).all(axis=1)
                out["derived"]["sequence_rows_finite"] = int(row_ok.sum())
                norms = np.linalg.norm(seq[row_ok], axis=1) if row_ok.any() else np.asarray([], dtype=np.float64)
                if norms.size:
                    out["derived"]["sequence_norms"] = {
                        "min": float(norms.min()),
                        "max": float(norms.max()),
...
def _try_plot_histograms(names: List[str], mat: np.ndarray, out_dir: Path, *, max_features: int) -> None:
    ...
    for j, name in enumerate(names):
        ...
        plt.hist(col, bins=min(20, max(5, col.size)), color="#0f766e", edgecolor="white")
        plt.title(name[:80])
        plt.tight_layout()
...
def _try_plot_corr_heatmap(names: List[str], corr: np.ndarray, out_path: Path) -> None:
    ...
    if F == 0:
        return
    fig, ax = plt.subplots(figsize=(max(7, F * 0.5), max(6, F * 0.5)))
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 NPZ stats for clap_extractor")
    p.add_argument("--result-store", type=Path, default=None)
    p.add_argument("--platform", default="youtube")
    p.add_argument("--component", default="clap_extractor")
    p.add_argument("--npz-name", default="clap_extractor_features.npz")
    ...
    json_path = args.out_dir / "clap_extractor_audit_v4_stats.json"
