...
<body>
    ...
    <div class="section">
        ...
        <div id="chroma-plot"></div>
    </div>
    
    <script>
        var chromaNames = {json.dumps(chroma_names)};
        var chromaMean = {json.dumps(render.get("basic_stats", {}).get("chroma_mean", []))};
        var chromaStd = {json.dumps(render.get("basic_stats", {}).get("chroma_std", []))};
        ...
        var layoutStats = {{
            title: 'Chroma Statistics',
            xaxis: {{ title: 'Chroma Class' }},
            yaxis: {{ title: 'Value' }},
            barmode: 'group',
            height: 400
        }};
        
        Plotly.newPlot('basic-stats-plot', [traceMean, traceStd], layoutStats);
        
        // Chroma spectrogram plot
        if (chroma !== null && chroma.length > 0) {{
            var chromaData = chroma.map(function(row, i) {{
                return {{
                    z: [row],
                    y: [chromaNames[i]],
                    type: 'heatmap',
            ...
            var layoutChroma = {{
                title: 'Chroma Spectrogram',
                xaxis: {{ title: 'Time (frames)' }},
                yaxis: {{ title: 'Chroma Class', tickvals: chromaNames.map(function(_, i) {{ return i; }}), ticktext: chromaNames }},
                height: 500
            }};
            
            Plotly.newPlot('chroma-plot', chromaData, layoutChroma);
        }} else {{
            document.getElementById('chroma-plot').innerHTML = '<p>Chroma time series not available (feature not enabled or saved to .npy file)</p>';
        }}
    </script>
</body>
</html>"""
