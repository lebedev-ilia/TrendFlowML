...
def render_emotion_diarization_extractor(npz_data: Dict[str, Any], meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    segments_total = safe_int(meta.get("segments_total", segments_count))
    render["summary"] = {
        "segments_count": segments_count,
        "segments_total": segments_total,
        "sample_rate": safe_int(meta.get("sample_rate", 16000)),
    }
...
<head>
    ...
    <style>
        ...
        .canvas-wrap {{ background: #fafafa; border: 1px solid #eee; border-radius: 8px; padding: 12px; }}
...
"""
    ...
    # Timeline visualization (offline, no-CDN)
    html_content += """
...
"""
    ...
    js_labels = json.dumps(emotion_labels)
    js_colors = json.dumps({str(k): v for k, v in emotion_colors.items()})
    html_content += f"""
        </div>
    </div>
    
    <script>
        const timeline = {js_timeline};
        ...
        function drawTimeline() {{
            ...
            const starts = timeline.map(t => (typeof t.start_sec === 'number' ? t.start_sec : (t.center_sec || 0)));
            const ends = timeline.map(t => (typeof t.end_sec === 'number' ? t.end_sec : (t.center_sec || 0)));
            ...
            for (const seg of timeline) {{
                const s = (typeof seg.start_sec === 'number' ? seg.start_sec : seg.center_sec);
                const e = (typeof seg.end_sec === 'number' ? seg.end_sec : seg.center_sec);
                const emo = (typeof seg.emotion_id === 'number' ? seg.emotion_id : -1);
                const conf = (typeof seg.emotion_confidence === 'number' ? seg.emotion_confidence : NaN);

                const x0 = clamp(((s - t0) / span) * W, 0, W);
                const x1 = clamp(((e - t0) / span) * W, 0, W);
                const w = Math.max(2, x1 - x0);

                let fill = '#cfcfcf';
                if (emo >= 0) {{
                    const c = colors[String(emo)] || '#666';
                    fill = c;
                }}
                const alpha = (isFinite(conf) ? clamp(conf, 0.15, 1.0) : 0.15);
