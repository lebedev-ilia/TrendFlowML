#!/usr/bin/env python3
"""
Статистика по NPZ `emotion_diarization_extractor` для Audit v4 / 4.2 (план §4).

Сводка по файлам, агрегат по прогонам, корреляции tabular, опционально PNG.
Учитывает object scalar dict-поля (через `.item()`).
"""
...
def _object_scalar_summary(v: Any) -> Dict[str, Any]:
    if isinstance(v, dict):
        keys = list(v.keys())
        key_types = sorted({type(k).__name__ for k in keys})
        return {
            "type": "dict",
            "n_keys": int(len(keys)),
            "key_types": key_types,
            "sample_keys": [str(k) for k in keys[:10]],
        }
    if isinstance(v, list):
        return {"type": "list", "len": int(len(v))}
...
def summarize_npz(path: Path) -> Dict[str, Any]:
    ...
    with np.load(path, allow_pickle=True) as data:
        ...
        for key in sorted(data.files):
            ...
            if arr.dtype == object:
                ...
                out["objects"][key] = {
                    ...
                    "value_summary": _object_scalar_summary(v) if v is not None else {"type": "unavailable"},
                }
            else:
                out["arrays"].append(_numeric_array_stats(arr, key))
        ...
        # Derived: mask consistency + ids/confidence stats
        ...
        if "emotion_id" in data.files:
            eid = np.asarray(data["emotion_id"]).astype(np.int64, copy=False).ravel()
            out["derived"]["emotion_id_neg1_frac"] = float(np.sum(eid == -1) / max(eid.size, 1))
            if eid.size:
                valid = eid[eid != -1]
                out["derived"]["emotion_id_unique_valid"] = int(len(np.unique(valid))) if valid.size else 0

        if "emotion_confidence" in data.files:
            conf = np.asarray(data["emotion_confidence"], dtype=np.float64).ravel()
            out["derived"]["confidence_nan_frac"] = float(np.isnan(conf).sum() / max(conf.size, 1))
            fin = conf[np.isfinite(conf)]
            if fin.size:
                out["derived"]["confidence"] = {
                    "min": float(fin.min()),
                    "max": float(fin.max()),
                    "mean": float(fin.mean()),
        ...
        if "emotion_probs" in data.files:
            probs = np.asarray(data["emotion_probs"], dtype=np.float64)
            if probs.ndim == 2 and probs.size:
                row_sum = np.nansum(probs, axis=1)
                fin = row_sum[np.isfinite(row_sum)]
                if fin.size:
                    out["derived"]["probs_row_sum"] = {
                        "min": float(fin.min()),
                        "max": float(fin.max()),
                        "mean": float(fin.mean()),
                        "p50": float(np.percentile(fin, 50)),
...
def _try_plot_histograms(names: List[str], mat: np.ndarray, out_dir: Path, *, max_features: int) -> None:
    ...
    for j, name in enumerate(names):
        ...
        plt.hist(col, bins=min(20, max(5, col.size)), color="#7c3aed", edgecolor="white")
        plt.title(name[:80])
        plt.tight_layout()
...
def _try_plot_corr_heatmap(names: List[str], corr: np.ndarray, out_path: Path) -> None:
    ...
    if F == 0:
        return
    fig, ax = plt.subplots(figsize=(max(7, F * 0.55), max(6, F * 0.55)))
    im = ax.imshow(corr, vmin=-1, vmax=1, cmap="RdBu_r")
    ax.set_xticks(range(F))
    ax.set_yticks(range(F))
    ax.set_xticklabels([n[:14] for n in names], rotation=90, fontsize=7)
    ax.set_yticklabels([n[:14] for n in names], fontsize=7)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 NPZ stats for emotion_diarization_extractor")
    p.add_argument("--result-store", type=Path, default=None)
    p.add_argument("--platform", default="youtube")
    p.add_argument("--component", default="emotion_diarization_extractor")
    p.add_argument("--npz-name", default="emotion_diarization_extractor_features.npz")
    ...
    json_path = args.out_dir / "emotion_diarization_extractor_audit_v4_stats.json"
