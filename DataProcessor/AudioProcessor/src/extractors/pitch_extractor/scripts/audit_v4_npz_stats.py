#!/usr/bin/env python3
"""
Статистика по NPZ `pitch_extractor` для Audit v4 / 4.2 (план §4).
...
def summarize_npz(path: Path) -> Dict[str, Any]:
    ...
    with np.load(path, allow_pickle=True) as data:
        ...
        if "segment_mask" in data.files:
            ...
            out["derived"]["segments_invalid"] = int(m.size - m.sum())
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 NPZ stats for pitch_extractor")
    p.add_argument("--result-store", type=Path, default=None)
    p.add_argument("--platform", default="youtube")
    p.add_argument("--component", default="pitch_extractor")
    p.add_argument("--npz-name", default="pitch_extractor_features.npz")
    ...
    json_path = args.out_dir / "pitch_extractor_audit_v4_stats.json"
