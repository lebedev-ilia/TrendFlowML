...
def render_key_extractor(npz_data: Dict[str, Any], meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Time series if enabled (Audit v3: prefer segment_center_sec, fallback segment_centers_sec)
    if "time_series" in features_enabled:
        segment_centers = npz_data.get("segment_center_sec") or npz_data.get("segment_centers_sec")
        ...
        segment_start = npz_data.get("segment_start_sec")
        segment_end = npz_data.get("segment_end_sec")
        ...
        segment_centers = _to_list(segment_centers)
        key_names_seq = _to_list(key_names_seq)
        key_modes_seq = _to_list(key_modes_seq)
        key_confidences_seq = _to_list(key_confidences_seq)
        if segment_durations is None and segment_start is not None and segment_end is not None:
            start_arr = np.asarray(segment_start)
            end_arr = np.asarray(segment_end)
            segment_durations = (end_arr - start_arr).tolist()
        else:
            segment_durations = _to_list(segment_durations)
...
def render_key_extractor_html(npz_path: str, output_path: str) -> str:
    ...
    # Audit v3: offline-only HTML (no Plotly CDN), vanilla canvas/SVG
...
<body>
    ...
    <div class="section">
        ...
        <canvas id="key-distribution-canvas" width="400" height="300"></canvas>
    </div>
    ...
    <div class="section">
        ...
        <canvas id="key-timeline-canvas" width="800" height="300"></canvas>
    </div>
    ...
    <div class="section">
        ...
        <canvas id="confidence-timeline-canvas" width="800" height="300"></canvas>
    </div>
