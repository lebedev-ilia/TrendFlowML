...
- `key_confidence`: уверенность в определении (float, 0.0-1.0, выше = увереннее)
- `method`: использованный метод ("essentia" | "librosa")
- `key_confidence_category`: категория уверенности ("high" | "medium" | "low" | "very_low")
- `key_low_confidence_warning`: флаг предупреждения о низкой уверенности (bool)
- `key_confidence_reason`: причина низкой уверенности ("normal" | "multiple_keys" | "atonal_or_insufficient_content" | "low_confidence")
...
- `key_top_k` зависит от `enable_top_k` и вычисления `key_scores` (если `enable_detailed_scores=True`, использует их; иначе вычисляет локально)
...
```python
{
    ...
    "key_method": "auto",           # "essentia" | "librosa" | "auto" (явный выбор метода)
...
- `--key-enable-time-series`: Включить временные серии (`segment_centers_sec`, `key_names_sequence`, и т.д.)
...
2. **Обработка каждого сегмента**: определение тональности для каждого сегмента
3. **Агрегация результатов**: определение доминирующей тональности (наиболее частая)
4. **Временные серии** (если `enable_time_series=True`): сохранение последовательностей по сегментам
5. **Детекция смены тональности** (если `enable_key_changes=True`): поиск переходов между тональностями
6. **Метрики стабильности** (если `enable_stability_metrics=True`): вычисление статистик по сегментам
...
result = key_extractor.run(input_uri, tmp_path, shared_features=shared_features)
