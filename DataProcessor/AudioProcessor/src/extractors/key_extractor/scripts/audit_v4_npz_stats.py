#!/usr/bin/env python3
"""
Статистика по NPZ `key_extractor` для Audit v4 / 4.2 (план §4).
...
def _parse_meta(meta_obj: Any) -> Dict[str, Any]:
    ...
    for k in (
        ...
        "key_name",
        "key_mode",
        "key_confidence_category",
        "key_low_confidence_warning",
    ):
        if k in meta_obj:
            flat[k] = meta_obj[k]
    return flat
...
def summarize_npz(path: Path) -> Dict[str, Any]:
    ...
    with np.load(path, allow_pickle=True) as data:
        ...
        if "key_id_by_segment" in data.files:
            kid = np.asarray(data["key_id_by_segment"]).astype(np.int64, copy=False).ravel()
            out["derived"]["key_id_neg1_frac"] = float(np.sum(kid < 0) / max(kid.size, 1))
            valid = kid[kid >= 0]
            out["derived"]["key_id_unique_valid"] = int(len(np.unique(valid))) if valid.size else 0

        if "key_confidence_by_segment" in data.files:
            kc = np.asarray(data["key_confidence_by_segment"], dtype=np.float64).ravel()
            fin = kc[np.isfinite(kc)]
            out["derived"]["key_confidence_nan_frac"] = float(np.isnan(kc).sum() / max(kc.size, 1))
            if fin.size:
                out["derived"]["key_confidence"] = {
                    "min": float(fin.min()),
                    "max": float(fin.max()),
                    "mean": float(fin.mean()),
...
def _try_plot_histograms(names: List[str], mat: np.ndarray, out_dir: Path, *, max_features: int) -> None:
    ...
    for j, name in enumerate(names):
        ...
        plt.hist(col, bins=min(20, max(5, col.size)), color="#2563eb", edgecolor="white")
        plt.title(name[:80])
        plt.tight_layout()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 NPZ stats for key_extractor")
    p.add_argument("--result-store", type=Path, default=None)
    p.add_argument("--platform", default="youtube")
    p.add_argument("--component", default="key_extractor")
    p.add_argument("--npz-name", default="key_extractor_features.npz")
    ...
    json_path = args.out_dir / "key_extractor_audit_v4_stats.json"
