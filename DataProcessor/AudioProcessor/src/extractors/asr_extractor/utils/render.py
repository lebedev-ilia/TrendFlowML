...
def render_asr_extractor(npz_data: Dict[str, Any], meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # ASR-internal profiler (stage 2): per-extractor timings + optional RSS/GPU snapshots
    asr_st = meta.get("asr_stage_timings_ms") if isinstance(meta, dict) else None
    if isinstance(asr_st, dict) and asr_st:
        render["summary"]["asr_stage_timings_ms"] = {str(k): safe_float(v, 0.0) for k, v in asr_st.items()}
    asr_rp = meta.get("asr_resource_profile") if isinstance(meta, dict) else None
    if isinstance(asr_rp, dict) and asr_rp:
        out_rp = {}
        for k, v in asr_rp.items():
            if v is None:
                out_rp[str(k)] = None
            else:
                try:
                    out_rp[str(k)] = safe_float(v, 0.0)
                except Exception:
                    out_rp[str(k)] = v
        render["summary"]["asr_resource_profile"] = out_rp
    ...
    lang_code_by_segment = npz_data.get("lang_code_by_segment")
    lang_conf_by_segment = npz_data.get("lang_conf_by_segment")
    ...
    if segment_center_sec is not None:
        ...
        if isinstance(lang_code_by_segment, np.ndarray):
            lang_code_by_segment = lang_code_by_segment.tolist()
        if isinstance(lang_conf_by_segment, np.ndarray):
            lang_conf_by_segment = lang_conf_by_segment.tolist()
        ...
        for i, center_sec in enumerate(segment_center_sec):
            ...
            if lang_code_by_segment and i < len(lang_code_by_segment):
                entry["lang_code"] = str(lang_code_by_segment[i] or "")
            if lang_conf_by_segment and i < len(lang_conf_by_segment):
                entry["lang_conf"] = safe_float(lang_conf_by_segment[i], None)
...
def render_asr_extractor_html(npz_path: str, output_path: str, decode_tokenizer: bool = True) -> str:
    ...
    # Generate HTML
    # NOTE(Audit v3): offline-only render. No CDN.
...
"""
    ...
    for i, entry in enumerate(render.get("timeline", [])):
        ...
        lang_conf = entry.get("lang_conf", None)
        ...
        html_content += f"""
            <div class="{segment_class}">
                <div class="segment-header">
                    ...
                    | Tokens: {token_count} | Lang: {html.escape(str(lang_code or ''))} ({'' if lang_conf is None else f'{float(lang_conf):.2f}'}) | Lang ID: {lang_id}
                </div>
"""
    ...
    # Offline-only: no Plotly/CDN. Render a minimal histogram using plain HTML.
    token_counts = [int(e.get("token_count", 0) or 0) for e in render.get("timeline", [])]
    if token_counts:
        # simple bins (0,1-50,51-100,...)
        bins = [(0, 0), (1, 50), (51, 100), (101, 200), (201, 400), (401, 800), (801, 999999)]
        counts = []
        for lo, hi in bins:
            c = sum(1 for x in token_counts if lo <= x <= hi)
            label = f"{lo}" if lo == hi else f"{lo}-{hi if hi < 999999 else 'inf'}"
            counts.append((label, c))
        html_content += "<h3>Token Counts (offline histogram)</h3>\n"
        html_content += "<pre style=\"background:#f7f7f7;padding:10px;border-radius:6px;\">\n"
        for label, c in counts:
            bar = "#" * min(60, c)
            html_content += f"{label:>10} | {bar} {c}\n"
        html_content += "</pre>\n"
