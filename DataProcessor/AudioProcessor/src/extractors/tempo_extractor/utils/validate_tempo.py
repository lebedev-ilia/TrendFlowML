#!/usr/bin/env python3
"""Валидатор tempo_extractor: схема NPZ, --struct (N, bpm_by_segment), --qa (meta + tabular)."""
...
def validate_schema(npz_path: str) -> bool:
    try:
        ...
        return "tempo_extractor_npz_v1" in sv or "tempo_extractor" in sv
    except Exception:
        return False
...
def validate_structure(npz_path: str) -> List[str]:
    out: List[str] = []
    d = load_npz(npz_path)
    has_seg = all(k in d for k in _SEGMENT)
    if not has_seg:
    ...
    bpm = d.get("bpm_by_segment")
    if bpm is not None and int(np.asarray(bpm).size) != n0:
        out.append(f"bpm_by_segment: длина {int(np.asarray(bpm).size)} != N={n0}")
    te = d.get("tempo_estimates")
    if te is not None:
        a = np.asarray(te, dtype=np.float64).ravel()
        if a.ndim > 1:
            out.append("tempo_estimates: ожидается 1D")
    fn, fv = d.get("feature_names"), d.get("feature_values")
    if fn is not None and fv is not None:
        try:
            names = [str(x) for x in np.asarray(fn, dtype=object).ravel()]
...
def main() -> int:
    p = argparse.ArgumentParser(description="validate tempo_extractor NPZ")
    p.add_argument("npz_path")
    p.add_argument(
        "--qa",
        action="store_true",
        help="Проверить плоский meta + tabular (view_csv_feature_qa.json → tempo_extractor).",
    )
    p.add_argument(
        "--struct",
        action="store_true",
        help="Согласованность N по сегментам и bpm_by_segment.",
    )
    args = p.parse_args()
