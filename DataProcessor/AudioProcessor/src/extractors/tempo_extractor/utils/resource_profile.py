...
def is_tempo_resource_profile_enabled() -> bool:
    v = os.getenv("AP_TEMPO_RESOURCE_PROFILE", "")
    return v.strip() in ("1", "true", "True", "yes", "on", "ON")


def capture_tempo_resource_profile(*, stage: str) -> Optional[Dict[str, Any]]:
    if not is_tempo_resource_profile_enabled():
        return None
