#!/usr/bin/env python3
"""
Статистика по NPZ `tempo_extractor` для Audit v4 / 4.2 (план §4).
"""
...
def summarize_npz(path: Path) -> Dict[str, Any]:
    ...
    with np.load(path, allow_pickle=True) as data:
        ...
        # Derived: axis sanity
        if "segment_center_sec" in data.files and "bpm_by_segment" in data.files:
            c = np.asarray(data["segment_center_sec"], dtype=np.float64).reshape(-1)
            b = np.asarray(data["bpm_by_segment"], dtype=np.float64).reshape(-1)
            out["derived"]["tempo_axis_n"] = int(c.size)
            out["derived"]["tempo_axis_match"] = bool(c.size == b.size)
            out["derived"]["bpm_by_segment_finite_frac"] = float(np.isfinite(b).sum() / max(b.size, 1))
...
def _try_plot_histograms(names: List[str], mat: np.ndarray, out_dir: Path, *, max_features: int) -> None:
    ...
    for j, name in enumerate(names):
        ...
        plt.hist(col, bins=min(24, max(6, col.size)), color="#f59e0b", edgecolor="white")
        plt.title(name[:80])
        plt.tight_layout()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Audit v4 NPZ stats for tempo_extractor")
    p.add_argument("--result-store", type=Path, default=None)
    p.add_argument("--platform", default="youtube")
    p.add_argument("--component", default="tempo_extractor")
    p.add_argument("--npz-name", default="tempo_extractor_features.npz")
    ...
    json_path = args.out_dir / "tempo_extractor_audit_v4_stats.json"
