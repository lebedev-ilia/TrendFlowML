"""
NPZ савер для voice_quality_extractor (Audit v3).
Canonical axis: segment_start_sec, segment_end_sec, segment_center_sec, segment_mask.
Per-segment: jitter_by_segment, shimmer_by_segment, hnr_by_segment.
"""
import os
import numpy as np
...
def save_voice_quality_npz(
    *,
    ...
    """
    Сохраняет NPZ артефакт для voice_quality_extractor (Audit v3: flat keys only).
    """
    ...
    # Metadata (tabular: numbers only; f0_method is str → meta)
    ...
    # Per-segment arrays
    jitter_by_segment = np.asarray(payload.get("jitter_by_segment") or [], dtype=np.float32).reshape(-1)
    shimmer_by_segment = np.asarray(payload.get("shimmer_by_segment") or [], dtype=np.float32).reshape(-1)
    hnr_by_segment = np.asarray(payload.get("hnr_by_segment") or [], dtype=np.float32).reshape(-1)

    # Time series (feature-gated, from .npy or inline)
    f0 = None
    amps = None
    hnr_vals = None
        ...
        meta=build_meta(
            ...
            extra={
                ...
                **({"error": error} if error else {}),
                ...
                "voice_quality_resource_profile": payload.get("voice_quality_resource_profile"),
