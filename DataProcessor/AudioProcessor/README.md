...
Схема: `schema_version="audio_npz_v1"` (см. `docs/contracts/ARTIFACTS_AND_SCHEMAS.md`).
  ...
  audio:
    ...
    extractors:
      ...
      asr:
        ...
        fallback_decode:
          enabled: false  # Включить fallback decode при низком avg_logprob
          temperature: 0.4  # Temperature для fallback decode
          avg_logprob_threshold: -1.0  # Порог avg_logprob для активации fallback
...
- **clap_extractor**: Timeline с `segment_centers_sec` и embedding norms, t-SNE визуализация (опционально)
